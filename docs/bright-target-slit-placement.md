...
**不适用范围：** 普通目标、目标身份不确定、没有新鲜 QHY WCS、C11 焦点证据过期、多个相近亮源、翼部被边缘或邻星截断
...
- **焦点证据：** 必须来自独立、仍在有效期内的 C11/Gemini（N.I.N.A. `Star Focuser Pro`）证据，源相机必须是锁定的 G3；当前饱和帧始终记录 `focusEligible=false`。
...
5. Night Setup 中恰好有一个 `C11Main` 焦点绑定；其 metric 必须是锁定 G3 相机给出的 `G3StellarShape`，证据 SHA-256、有效期和置信度均通过。
