...
PHD2/G3 亮目标现场选星不得使用全画幅亮度优先或把 PHD2 多星圆圈当作恒星身份
证明。每个新 guide epoch 必须从同一张 fresh G3 帧提取目标和候选，排除超亮目标
光晕保护区、非紧致 FWHM、高椭率、饱和、边缘和狭缝邻域；`exact=false` 选星被
PHD2 接受后，`guide` 的后备搜索只允许在该质心附近的有界 ROI 内进行。2026-08-23
本机以 `80×80 px` 围栏完成实机闭环；这是当前 G3 profile 的 commissioning 证据，
不是跨 ROI/binning 的机器无关星点尺寸。若同帧确实没有独立星，显式退化到已单独
commissioning 的短曝光直接目标导星；不得回到全幅光晕选星，也不得把短曝光用作
对焦证据。完整现场结论见 [2026-08-23 实机调试收口](commissioning-night-2026-08-23.md)。
