...
- continues a settled lineage across process or run handoff without resetting cumulative distance, action count, the commissioning-wide ceilings, or the earliest elapsed-time origin. A WCS-to-local-search family handoff re-attests the same immutable commissioning hash, tightens single-command/radius, and caps cumulative distance, attempts, and elapsed time at `already consumed + current-family increment`, never above the commissioning-wide ceiling. This lets a declared fallback use the remaining global action budget even when the preceding WCS family had a smaller family cap, without minting a new lineage or resetting any consumed action.
...
The same charged-position handoff applies to the single bounded G3/PL3 reacquisition performed after PHD2 recalibration or a failed initial guide/placement quality window. A successfully settled WCS position can legitimately remain far from the original lineage origin; if its fresh reported coordinate is still within the handoff envelope of both the last charged readback and the recorded TAN offset, the reacquisition continues that charged lineage rather than demanding an impossible return/rebase. All consumed motion, actions and elapsed time remain intact.

An overlapping-field PL3 solution may also be the final formal WCS needed before PHD2 hand-off. After its durably charged return toward the runtime slit, the target field can legitimately fail to solve because an ultra-bright or sparse target dominates the G3 frame. The same `G3PostWcsPredictionPolicy` therefore evaluates the fresh immutable target frame against the preceding formal PL3/mount prediction and the unchanged detector-fixed runtime slit, whether slit geometry was measured in that call or reused from the run cache. Frame inclusion, prediction uncertainty and target-to-slit residual must all fit the commissioned PHD2 acquisition window. Passing produces `G3_FIELD_ANALYZED_MOTION_PREDICTED` without another target-field solve or repeated HDR; failure permits the already-ledgered safe return but is not recoverable by restarting the same generic neighbouring search lineage.
