...
6. By default, do not begin a science spectral exposure while guiding is unconfirmed or unstable, or while the fresh target/slit residual is outside tolerance. The explicitly owner-approved supervised exception in ADR-0013 permits ATR probing and spectrum-quality-gated continuation with measured slit-precision/settle warnings; under ADR-0015 this scientific-quality choice is saved per N.I.N.A. Profile and displayed in the run-control header, while real-device control arming remains per-session. New profiles remain strict. The exception never grants exact-placement or unattended-quality status, and does not waive identity, current guide/lock evidence, device safety or motion accounting.
