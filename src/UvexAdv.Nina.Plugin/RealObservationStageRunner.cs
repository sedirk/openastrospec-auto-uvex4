...
internal sealed partial class RealObservationStageRunner : ObservationStageRunnerBase, IObservationRunProvenanceSource, IRealObservationRunOwnershipSource, IAsyncDisposable
{
    ...
    // storm when a meridian/limit condition is genuinely refusing tracking.
    private static readonly TimeSpan MountTrackingEnableTimeout = TimeSpan.FromSeconds(15);
    private static readonly TimeSpan MountTrackingEnablePollInterval = TimeSpan.FromMilliseconds(250);
    ...
    private readonly SemaphoreSlim environmentMechanicalGate = new(1, 1);
    ...
    private bool qhyAvailableForRun = true;
    private string? qhyUnavailableReason;
    ...
    private int qhyMountCoordinateSyncPerformed;
    private int atrScienceTemperatureRequired;
    private int atrStableTemperatureEstablished;
    private int domeOrRoofOpenEstablished;
    private int domeOrRoofLifecycleCommitted;
    private int environmentSafetyShutdownStarted;
    private string? environmentSafetyTrip;
    private Task environmentSafetyShutdownTask = Task.CompletedTask;
    ...
    private string? qhyG3FastPairCollectionRunId;
    ...
    private G3SlitGeometryRunCache? g3SlitGeometryRunCache;
        ...
        IProgress<ApplicationStatus> progress)
    {
        ...
        {
            // QHY is an optional synchronized-photometry/wide-field branch.  A
            // malformed optional endpoint must not prevent construction of the
            // required ATR/G3/UVEX spectroscopy runner.  The inert loopback
            // client is never contacted while qhyAvailableForRun is false.
            qhy = new QhyServiceClient("http://127.0.0.1:1/");
        ...
        phd2 = new Phd2Client(new Phd2ClientOptions
        {
            ...
            ReadConfigurationFingerprint = configuration.Phd2Port == 4400
                ? () => WindowsPhd2ConfigurationFingerprint.Read(configuration.Phd2.ProfileId)
                : null,
        });
        safetyMonitorMediator.IsSafeChanged += OnSafetyMonitorSafeChanged;
    ...
    public ObservationRunLockedMetadata LockedMetadata
    {
        get
        {
            ...
            labels["effectiveSupervisedScienceOptIn"] = HasSupervisedScienceOptIn().ToString(CultureInfo.InvariantCulture);
            labels["allowSupervisedSlitQualityWarning"] = configuration.AllowSupervisedSlitQualityWarning.ToString(CultureInfo.InvariantCulture);
            ...
            labels["qhyServiceAvailableAtRunLock"] = (qhyProof is not null).ToString(CultureInfo.InvariantCulture);
            if (qhyProof is not null)
            {
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            try
            {
                ...
                if (durableG3Recovery is not null)
                {
                    return await CompleteStageResultAsync(stage, durableG3Recovery, cancellationToken).ConfigureAwait(false);
                }
                var durableSlitRecovery = await RecoverDurableSlitPlacementBeforeStageAsync(
                    stage,
                    context,
                    cancellationToken).ConfigureAwait(false);
                if (durableSlitRecovery is not null)
                {
                ...
                var resumeRecovery = await RecoverInterruptedStageAsync(
                    ...
                    cancellationToken).ConfigureAwait(false);
                if (resumeRecovery is not null)
                {
                    var nestedAutomaticRecovery = await PrepareAutomaticStageRecoveryAsync(
                        resumeRecovery.FailedStage,
                        resumeRecovery.Result,
                        automaticRecoverySession,
                        context,
                        cancellationToken).ConfigureAwait(false);
                    if (nestedAutomaticRecovery.Retry)
                    {
                        // The failed inner stage has been invalidated according
                        // to its exact reviewed policy. Re-arm the outer chain
                        // so a retry can never skip the failed prerequisite.
                        Interlocked.Exchange(ref resumeRecoveryRequired, 1);
                        continue;
                    }
                    return await CompleteStageResultAsync(
                        stage,
                        nestedAutomaticRecovery.Result,
                        cancellationToken).ConfigureAwait(false);
                }
                var result = stage switch
                {
                    ...
                    ObservationStage.AcquireQhyWideField => await AcquireOptionalQhyWideFieldAsync(context, cancellationToken).ConfigureAwait(false),
                    ...
                    ObservationStage.StartQhyPhotometry => await StartOptionalQhyPhotometryAsync(context, cancellationToken).ConfigureAwait(false),
            ...
            catch (Exception ex)
            {
                ...
                var cleanup = await CleanupAfterFailureAsync(
                    $"{stage}: {ex.Message}",
                    cancellationToken,
                    // In weak supervision this remains a stop-only pause for
                    // operator inspection. In full unattended mode, a roof
                    // that was opened/accepted by this run must not be left
                    // open after a stage exception.
                    allowMechanicalActions:
                        !configuration.Environment.WeakSupervisionEnabled &&
                        Volatile.Read(ref domeOrRoofOpenEstablished) != 0).ConfigureAwait(false);
    ...
    private async Task<(bool Retry, StageResult Result)> PrepareAutomaticStageRecoveryAsync(
        ObservationStage stage,
        StageResult result,
        ObservationAutomaticRecoverySession session,
        ObservationContext context,
        CancellationToken cancellationToken)
    {
        var decision = session.Evaluate(stage, result.Gate);
        ...
        {
            var exhausted = result with
            {
                Gate = result.Gate with
                {
                    Message =
                        $"{result.Gate.Message} Automatic recovery '{plan.Action}' exhausted " +
                        $"its {plan.MaximumAttempts} exact stage/code attempt(s) or the " +
                        $"{ObservationAutomaticRecoverySession.MaximumTotalAttempts}-attempt session fence; " +
                        $"durable motion and action budgets were not reset.",
                },
            };
            await WriteAuditBestEffortAsync("automatic-stage-recovery-exhausted", new
            {
                stage = stage.ToString(),
                result.Gate.Code,
                action = plan.Action.ToString(),
                consumedAttempts = decision.AttemptNumber,
                totalAttempts = decision.TotalAttempts,
                plan.MaximumAttempts,
                plan.Reason,
        ...
        switch (plan.Action)
        {
            case ObservationAutomaticRecoveryAction.RetryWithFreshStageEvidence:
                InvalidateStageState(stage);
                break;
            case ObservationAutomaticRecoveryAction.RebuildStageDependencies:
                if (stage is ObservationStage.SelectAtrExposure or ObservationStage.RunScienceBlock)
                {
                    ...
                    if (result.Gate.Code is "GUIDING_UNSTABLE" or "GUIDING_LOST" or "GUIDING_NOT_STABLE" or "PHD2_SCIENCE_GUIDE_EPOCH_CHANGED")
                    {
                        try
                        {
                            if (await TryVerifyScienceInPlaceAsync(context, cancellationToken).ConfigureAwait(false))
                                return (true, result);
                        }
                    ...
                    if (replacementBudget is not null) return (false, new StageResult(replacementBudget));
                }
            ...
            case ObservationAutomaticRecoveryAction.RetryTerminalCleanup:
                break;
            default:
                return (false, result);
        }
        ...
        Report(
            $"warning：{stage} 的 {result.Gate.Code} 将自动恢复 " +
            $"({plan.Action} {decision.AttemptNumber}/{plan.MaximumAttempts}, " +
            $"session {decision.TotalAttempts}/{ObservationAutomaticRecoverySession.MaximumTotalAttempts})；不会重置运动/动作账本");
        await WriteAuditBestEffortAsync("automatic-stage-recovery-started", new
        {
            stage = stage.ToString(),
            result.Gate.Code,
            ...
            totalAttempt = decision.TotalAttempts,
            plan.MaximumAttempts,
            delayMilliseconds = plan.Delay.TotalMilliseconds,
            plan.Reason,
            durableMotionBudgetsReset = false,
            automaticPhysicalActionResendAuthorized = false,
        }).ConfigureAwait(false);
    ...
    public override async Task<GateResult> RevalidateAsync(ObservationContext context, CancellationToken cancellationToken)
    {
        ...
        if (validation.Disposition != GateDisposition.Passed &&
            !configuration.Environment.WeakSupervisionEnabled &&
            Volatile.Read(ref domeOrRoofOpenEstablished) != 0)
        {
            var cleanup = await CleanupAfterFailureAsync(
                $"Interlock revalidation failed: {validation.Code}: {validation.Message}",
                cancellationToken,
            ...
            if (cleanup.Count > 0)
            {
                ...
                {
                    Message = $"{validation.Message} Full-unattended fail-safe cleanup was incomplete: {string.Join("; ", cleanup)}",
                };
            }
        }
        return validation;
    ...
    private async Task<StageResult> CompleteStageResultAsync(
        ObservationStage stage,
        StageResult result,
        CancellationToken cancellationToken)
    {
        if (result.Gate.Disposition == GateDisposition.Passed ||
            configuration.Environment.WeakSupervisionEnabled ||
            Volatile.Read(ref domeOrRoofOpenEstablished) == 0)
        {
            return result;
        ...
        var cleanup = await CleanupAfterFailureAsync(
            $"Full-unattended stage {stage} stopped: {result.Gate.Code}: {result.Gate.Message}",
            cancellationToken,
            allowMechanicalActions: true).ConfigureAwait(false);
        await WriteAuditBestEffortAsync("full-unattended-stage-stop-safe-close", new
        {
            stage = stage.ToString(),
            result.Gate.Code,
            disposition = result.Gate.Disposition.ToString(),
        ...
        var suffix = cleanup.Count == 0
            ? " Full-unattended fail-safe cleanup stopped data owners, handled the optical cover, parked the mount and attested roof closure."
            : $" Full-unattended fail-safe cleanup was incomplete: {string.Join("; ", cleanup)}";
        return result with { Gate = result.Gate with { Message = result.Gate.Message + suffix } };
    }
    ...
    public override async Task OnPausedAsync(ObservationContext context, CancellationToken cancellationToken)
    {
        ...
        var qhyWarnings = new List<string>();
        ...
        foreach (var id in activeQhyJobs.Keys.ToArray())
        {
            try
            {
                ...
                {
                    var warning = $"QHY job {id:D} pause state is {after?.State.ToString() ?? "missing"}.";
                    qhyWarnings.Add(warning);
                    DisableQhyForRun(warning);
                }
            }
            catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested) { throw; }
            catch (Exception ex)
            {
                var warning = $"QHY job {id:D} pause failed: {ex.Message}";
                qhyWarnings.Add(warning);
                DisableQhyForRun(warning);
            }
        }
        ...
        if (qhyWarnings.Count > 0)
        {
            Report($"warning：可选 QHY 通道未能确认暂停，已隔离并保留终态清理句柄；主光谱安全暂停不受阻（{string.Join("; ", qhyWarnings)}）");
        }
    ...
    public override async Task OnResumingAsync(ObservationContext context, CancellationToken cancellationToken)
    {
        ...
        foreach (var id in activeQhyJobs.Keys.ToArray())
        {
            try
            {
                ...
                else if (before?.State is QhyJobState.Paused or QhyJobState.PausedNeedsAttention)
                {
                    ...
                    {
                        var warning = $"QHY job {id:D} resume withheld: {immediate.Code}: {immediate.Message}";
                        qhyWarnings.Add(warning);
                        DisableQhyForRun(warning);
                        continue;
                    }
                ...
                if (!validResumeState)
                {
                    var warning = $"QHY job {id:D} resume state is {after?.State.ToString() ?? "missing"}.";
                    qhyWarnings.Add(warning);
                    DisableQhyForRun(warning);
                }
            }
            ...
            {
                var warning = $"QHY job {id:D} resume failed: {ex.Message}";
                qhyWarnings.Add(warning);
                DisableQhyForRun(warning);
            }
        }
        if (qhyWarnings.Count > 0)
        {
            await WriteAuditBestEffortAsync("qhy-resume-degraded", new
            {
                qhyWarnings,
                spectroscopyResumeContinues = true,
                lifecycleHandlesRetainedForFinalization = true,
            }).ConfigureAwait(false);
            Report($"warning：可选 QHY 通道恢复失败，已降级为无同步测光继续（{string.Join("; ", qhyWarnings)}）");
        }
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        try { await environmentSafetyShutdownTask.ConfigureAwait(false); } catch { /* The emergency audit retains the failure. */ }
        ...
        cleanupGate.Dispose();
        environmentMechanicalGate.Dispose();
    }
    ...
    private void OnSafetyMonitorSafeChanged(object? sender, IsSafeEventArgs args)
    {
        ...
        var trip = $"The locked N.I.N.A. Safety Monitor reported unsafe at {DateTimeOffset.UtcNow:O}. " +
                   "This run cannot automatically resume even if the monitor later returns safe.";
        Volatile.Write(ref environmentSafetyTrip, trip);

        // Abort the N.I.N.A.-owned ATR exposure synchronously so a long frame
        // does not delay the independent fail-safe cleanup path.  The raw
        // partial/aborted frame remains under N.I.N.A.'s normal policy.
        try { cameraMediator.AbortExposure(); }
        catch { /* The asynchronous cleanup records all attestable outcomes. */ }

        if (Volatile.Read(ref domeOrRoofOpenEstablished) == 0 ||
            Interlocked.CompareExchange(ref environmentSafetyShutdownStarted, 1, 0) != 0)
        {
            return;
        }

        environmentSafetyShutdownTask = Task.Run(async () =>
        {
            using var timeout = new CancellationTokenSource(TimeSpan.FromSeconds(
                configuration.Environment.DomeOrRoofTransitionTimeoutSeconds +
                configuration.Environment.OpticalCoverTransitionTimeoutSeconds + 120));
            IReadOnlyList<string> failures;
            try
            {
                failures = await CleanupAfterFailureAsync(
                    trip,
                    timeout.Token,
                    allowMechanicalActions: true).ConfigureAwait(false);
            }
            ...
            {
                failures = new[] { $"Safety-triggered cleanup raised {ex.GetType().Name}: {ex.Message}" };
            }

            await WriteAuditBestEffortAsync("environment-safety-trip", new
            {
                trip,
                failures,
                atrAbortRequested = true,
                fullUnattendedMode = true,
            }).ConfigureAwait(false);
        });
    }
    ...
    private async Task<StageResult> ValidateNightSetupAsync(ObservationContext context, CancellationToken cancellationToken)
    {
        var gate = await EvaluateInterlocksAsync(
            ...
            connectQhy: true,
            cancellationToken,
            connectUvex: true).ConfigureAwait(false);
        ...
        return gate.Severity == GateSeverity.Warning
            ? Warning(
                "REAL_NIGHT_SETUP_LOCKED_WITH_OPTIONAL_DEGRADATION",
                $"Night Setup '{context.Plan.NightSetupId}' passed every required gate. {gate.Message}",
                metrics,
                Metadata(preset))
            : Passed(
        ...
        bool connectUvex = false)
    {
        ...
        var advisoryWarnings = new List<string>();
        ...
        if (Volatile.Read(ref environmentSafetyTrip) is { } safetyTrip)
        {
            return GateResult.Fail(
                "ENVIRONMENT_SAFETY_TRIPPED",
                safetyTrip);
        }
        ...
        if (!configuration.MatchesCurrentProfile(
                ...
                CaptureCurrentEnvironmentDeviceSelection(),
                out var currentConfigurationSha256,
                out var profileDifferences))
        {
            return GateResult.Unknown(
                "REAL_PROFILE_DRIFT",
                $"An action-bearing N.I.N.A. Profile value changed after the real runner was created. Locked {configuration.ActionConfigurationSha256}, current {currentConfigurationSha256}. Changed: {profileDifferences}. Start a new run after deliberate revalidation.");
        }
        if (!qhyLeaseFailures.IsEmpty)
        {
            DisableQhyForRun($"QHY control lease became unhealthy: {string.Join(" ", qhyLeaseFailures.Values)}");
            advisoryWarnings.Add(qhyUnavailableReason!);
        }
        ...
        if (qhyAvailableForRun)
        {
            ...
            if (qhyServiceConfigurationGate.Severity == GateSeverity.Warning ||
                qhyServiceConfigurationGate.Disposition != GateDisposition.Passed)
            {
                DisableQhyForRun($"{qhyServiceConfigurationGate.Code}: {qhyServiceConfigurationGate.Message}");
                advisoryWarnings.Add(qhyUnavailableReason!);
            }
        }
        else if (!string.IsNullOrWhiteSpace(qhyUnavailableReason))
        {
            advisoryWarnings.Add(qhyUnavailableReason);
        }
        ...
        if (connectionGate.Severity == GateSeverity.Warning)
        {
            advisoryWarnings.Add($"{connectionGate.Code}: {connectionGate.Message}");
        }
        ...
        using (var uvex = new UvexServiceClient(configuration.UvexServiceUrl))
        {
            try
            {
                uvexStatus = connectUvex
                    ? await ConnectUvexAtCheckpointAsync(context, uvex, cancellationToken).ConfigureAwait(false)
                    : await uvex.GetStatusAsync(cancellationToken).ConfigureAwait(false);
            }
            catch (OperationCanceledException)
            {
                throw;
            ...
            catch (Exception ex)
            {
                return GateResult.Unknown(
                    ...
                    $"UVEX4 could not be connected and verified at the Night Setup boundary: {ex.Message}");
            }
            var uvexGate = ValidateUvexStatus(uvexStatus);
            if (uvexGate.Disposition != GateDisposition.Passed) return uvexGate;
        }

        var roofOpenGate = await EnsureDomeOrRoofOpenForUnattendedAsync(
            context,
            plan,
            cancellationToken).ConfigureAwait(false);
        if (roofOpenGate.Disposition != GateDisposition.Passed) return roofOpenGate;

        var environmentGate = ValidateEnvironment(plan);
        if (environmentGate.Disposition != GateDisposition.Passed) return environmentGate;
        if (environmentGate.Severity == GateSeverity.Warning)
        {
            advisoryWarnings.Add($"{environmentGate.Code}: {environmentGate.Message}");
        }

        QhyCameraStatus? qhyState = null;
        if (qhyAvailableForRun)
        {
            try
            {
            ...
            {
                DisableQhyForRun($"QHY connection/status failed: {ex.Message}");
                advisoryWarnings.Add(qhyUnavailableReason!);
            }
            if (qhyState is null || !qhyState.Connected || qhyState.Identity is null)
            {
                DisableQhyForRun("The dedicated QHY service does not report a connected camera.");
                advisoryWarnings.Add(qhyUnavailableReason!);
            }
            else if (!string.Equals(qhyState.Identity.StableId, plan.ExpectedQhyCameraId, StringComparison.Ordinal) ||
                     !qhyState.Identity.Model.Contains("QHYminiCam8", StringComparison.OrdinalIgnoreCase))
            {
                DisableQhyForRun(
                    $"The connected optional QHY device '{qhyState.Identity.Model}/{qhyState.Identity.StableId}' does not match '{plan.ExpectedQhyCameraId}'; it will not be controlled.");
                advisoryWarnings.Add(qhyUnavailableReason!);
                qhyState = null;
            }
        }
        ...
        var qhySetupState = qhyState ?? new QhyCameraStatus(
            ...
            Identity: null,
            TemperatureC: null,
            CoolerPowerPercent: null,
            LastError: qhyUnavailableReason,
        ...
        {
            advisoryWarnings.Add($"{deferred.Code}: {deferred.Message}");
        }
        ...
        if (coverGate.Severity == GateSeverity.Warning)
        {
            advisoryWarnings.Add($"{coverGate.Code}: {coverGate.Message}");
        }
        return advisoryWarnings.Count > 0
            ? GateResult.Warn(
                "REAL_INTERLOCKS_VALID_WITH_OPTIONAL_DEGRADATION",
                $"Required motion, explicit safety, ATR, G3/PHD2 and UVEX interlocks passed. Advisory degradations: {string.Join(" ", advisoryWarnings.Distinct(StringComparer.Ordinal))}",
                mountClockGate.Metrics)
            : GateResult.Pass(
                "REAL_INTERLOCKS_VALID",
                "All required real-mode identities, mount UTC, optical-cover state, safety states, setup values and commissioning provenance passed.",
                mountClockGate.Metrics);
    }
    ...
    private QhyServiceHealth? LoadInitialQhyServiceHealth()
    {
        try
        {
            ...
            if (issues.Count == 0) return health;
            DisableQhyForRun($"QHY service proof was not usable at run lock: {string.Join(" ", issues)}");
        }
        catch (Exception ex)
        {
            DisableQhyForRun($"QHY service was unavailable at run lock: {ex.Message}");
        }
        return null;
    }
        ...
        LiveFocusMetricState? currentMetric)
    {
        ...
        return IsGs350LockedNoMove(setup);
    }

    private static bool IsGs350LockedNoMove(NightSetupRecord setup)
    {
    ...
    private void DisableQhyForRun(string reason)
    {
        qhyAvailableForRun = false;
        qhyUnavailableReason ??= reason;
        ...
        // Disable scientific use without discarding the lifecycle handle.  A
        // degraded QHY photometry job may still own the physical camera and
        // must remain cancellable during checked finalization.
    }

    private static bool IsOptionalQhyLiveSetupGate(GateResult gate) =>
        gate.Code.StartsWith("QHY_", StringComparison.Ordinal) ||
        gate.Code.StartsWith("FOCUS_GS350_WIDE_FIELD_", StringComparison.Ordinal);

    private static bool IsDeferredAtrCaptureAttestationGate(GateResult gate) =>
        gate.Disposition == GateDisposition.Indeterminate &&
        gate.Code is "ATR_READOUT_MODE" or "ATR_ROI";
    ...
    private async Task<GateResult> ValidateQhyServiceConfigurationAsync(
        ...
        CancellationToken cancellationToken)
    {
        var localIssues = ValidateOptionalQhyConfiguration(plan);
        if (localIssues.Count > 0)
        {
            return GateResult.Fail(
                "QHY_OPTIONAL_CONFIGURATION_INVALID",
                string.Join(" ", localIssues));
        }
        ...
        var lockedHealth = lockedQhyServiceHealth.Value;
        if (lockedHealth is null)
        {
            return GateResult.Warn(
                "QHY_OPTIONAL_SERVICE_NOT_LOCKED",
                "QHY was unavailable when the immutable run manifest was created. Synchronized photometry and the wide-field witness are disabled for this run; fresh G3 acquisition and ATR spectroscopy may continue.");
        }
        var locked = lockedHealth.Configuration;
    ...
    private IReadOnlyList<string> ValidateOptionalQhyConfiguration(ObservationPlan plan)
    {
        var issues = new List<string>();
        if (string.IsNullOrWhiteSpace(plan.ExpectedQhyCameraId) ||
            plan.ExpectedQhyCameraId.StartsWith("SIM-", StringComparison.OrdinalIgnoreCase))
        {
            issues.Add("A real QHY StableId was not configured.");
        }
        if (configuration.Qhy.FocalLengthMillimeters <= 0 ||
            configuration.Qhy.PixelSizeMicrometers <= 0)
        {
            issues.Add("Measured GS350/QHY focal length and pixel size are unavailable.");
        }
        if (configuration.Qhy.ReadoutMode < 0 ||
            configuration.Qhy.RoiX < 0 ||
            configuration.Qhy.RoiY < 0 ||
            configuration.Qhy.RoiWidth < 0 ||
            configuration.Qhy.RoiHeight < 0)
        {
            issues.Add("QHY readout mode or ROI is invalid.");
        }
        if (configuration.Qhy.AcquisitionExposureLadderSeconds.Count == 0)
        {
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            // Environment adapters are part of the locked action configuration.
            // Connecting them is read-only with respect to the roof and cover:
            // no shutter, cover or mount command is issued here.  Do this before
            // opening any acquisition owner so full unattended runs never rely
            // on the operator having clicked three separate Connect buttons.
            var environmentConnectionGate = await EnsureNinaEnvironmentAdaptersConnectedAsync(
                context,
                cancellationToken).ConfigureAwait(false);
            if (environmentConnectionGate.Disposition != GateDisposition.Passed)
            {
                return environmentConnectionGate;
            }
            ...
            return GateResult.Pass(
                "NINA_EQUIPMENT_CONNECTED",
                "N.I.N.A. environment adapters plus ATR, telescope, C11 Star Focuser Pro and PHD2 guider were connected according to the locked supervision mode; live identity and state gates still apply.");
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
    ...
    private NinaEnvironmentDeviceSelection CaptureCurrentEnvironmentDeviceSelection()
    {
    ...
    private GateResult ValidateCurrentEnvironmentDeviceSelections()
    {
        var current = CaptureCurrentEnvironmentDeviceSelection();
        return current == configuration.Environment.Devices
            ? configuration.Environment.Devices.ValidateForMode(configuration.Environment.WeakSupervisionEnabled)
            : GateResult.Fail(
                "NINA_ENVIRONMENT_DEVICE_SELECTION_DRIFT",
                $"N.I.N.A. environment selections changed after run lock. " +
                $"Safety '{configuration.Environment.Devices.SafetyMonitorId}' -> '{current.SafetyMonitorId}', " +
                $"roof '{configuration.Environment.Devices.DomeOrRoofId}' -> '{current.DomeOrRoofId}', " +
                $"weather '{configuration.Environment.Devices.WeatherDataId}' -> '{current.WeatherDataId}', " +
                $"cover '{configuration.Environment.Devices.OpticalCoverId}' -> '{current.OpticalCoverId}'. Start a new run before any physical action.");
    }
    ...
    private async Task<GateResult> EnsureNinaEnvironmentAdaptersConnectedAsync(
        ...
        CancellationToken cancellationToken)
    {
        var selectionGate = ValidateCurrentEnvironmentDeviceSelections();
        if (selectionGate.Disposition != GateDisposition.Passed) return selectionGate;

        var devices = configuration.Environment.Devices;
        var warnings = new List<string>();

        if (!devices.HasSafetyMonitor)
        {
            warnings.Add("no Safety Monitor is selected");
        }
        else if (!safetyMonitorMediator.GetInfo().Connected)
        {
            await CheckpointAndRejectStaleStageStackAsync(context, cancellationToken).ConfigureAwait(false);
            selectionGate = ValidateCurrentEnvironmentDeviceSelections();
            if (selectionGate.Disposition != GateDisposition.Passed) return selectionGate;
            Report("按锁定的 N.I.N.A. Profile 自动连接安全监视器（只回读，不移动设备）");
            if (!await safetyMonitorMediator.Connect().ConfigureAwait(false))
            {
                if (configuration.Environment.RequireSafetyMonitor)
                {
                    return GateResult.Unknown("SAFETY_MONITOR_CONNECT_FAILED", "N.I.N.A. could not connect the selected Safety Monitor.");
                }
                warnings.Add("selected Safety Monitor could not connect");
            }
        }

        if (!devices.HasWeatherData)
        {
            warnings.Add("no weather adapter is selected");
        }
        else if (!weatherDataMediator.GetInfo().Connected)
        {
            await CheckpointAndRejectStaleStageStackAsync(context, cancellationToken).ConfigureAwait(false);
            selectionGate = ValidateCurrentEnvironmentDeviceSelections();
            if (selectionGate.Disposition != GateDisposition.Passed) return selectionGate;
            Report("按锁定的 N.I.N.A. Profile 自动连接天气源（只回读，不移动设备）");
            if (!await weatherDataMediator.Connect().ConfigureAwait(false))
            {
                if (configuration.Environment.RequireWeatherData)
                {
                    return GateResult.Unknown("WEATHER_CONNECT_FAILED", "N.I.N.A. could not connect the selected weather adapter.");
                }
                warnings.Add("selected weather adapter could not connect");
            }
        }

        if (!devices.HasDomeOrRoof)
        {
            warnings.Add("no dome/roll-off-roof adapter is selected");
        }
        else if (!domeMediator.GetInfo().Connected)
        {
            await CheckpointAndRejectStaleStageStackAsync(context, cancellationToken).ConfigureAwait(false);
            selectionGate = ValidateCurrentEnvironmentDeviceSelections();
            if (selectionGate.Disposition != GateDisposition.Passed) return selectionGate;
            Report("按锁定的 N.I.N.A. Profile 自动连接圆顶/平移顶适配器（本步骤不发开关顶命令）");
            if (!await domeMediator.Connect().ConfigureAwait(false))
            {
                if (configuration.Environment.RequireOpenDomeOrRoof)
                {
                    return GateResult.Unknown("ROOF_ADAPTER_CONNECT_FAILED", "N.I.N.A. could not connect the selected dome/roll-off-roof adapter.");
                }
                warnings.Add("selected dome/roll-off-roof adapter could not connect");
            }
        }

        // A full-unattended run also assumes terminal responsibility for a
        // roof that was already open when its exact locked adapter connected.
        // Without this adoption, a later camera/focuser/guider connection
        // failure could leave an already-open observatory outside the shared
        // failure-close path merely because the dedicated open stage had not
        // been reached yet.
        if (!configuration.Environment.WeakSupervisionEnabled && devices.HasDomeOrRoof)
        {
            var connectedRoof = domeMediator.GetInfo();
            if (connectedRoof.Connected &&
                EnvironmentAdapterIdentityMatches(connectedRoof.DeviceId, devices.DomeOrRoofId) &&
                connectedRoof.ShutterStatus == ShutterState.ShutterOpen)
            {
                Interlocked.Exchange(ref domeOrRoofOpenEstablished, 1);
                Interlocked.Exchange(ref domeOrRoofLifecycleCommitted, 1);
            }
        }

        if (!devices.HasOpticalCover)
        {
            warnings.Add("no optical-cover adapter is selected");
        }
        else if (!flatDeviceMediator.GetInfo().Connected)
        {
            await CheckpointAndRejectStaleStageStackAsync(context, cancellationToken).ConfigureAwait(false);
            selectionGate = ValidateCurrentEnvironmentDeviceSelections();
            if (selectionGate.Disposition != GateDisposition.Passed) return selectionGate;
            Report(configuration.Environment.RequireOpenOpticalCover
                ? "按锁定的 N.I.N.A. Profile 自动连接主光路电动镜盖"
                : "有人弱监督：检测到镜盖适配器选择，自动连接并使用可回读的状态");
            if (!await flatDeviceMediator.Connect().ConfigureAwait(false))
            {
                if (configuration.Environment.RequireOpenOpticalCover)
                {
                    return GateResult.Unknown("OPTICAL_COVER_CONNECT_FAILED", "N.I.N.A. could not connect the selected flat-device/cover adapter.");
                }
                warnings.Add("selected optical-cover adapter could not connect");
            }
        }

        if (warnings.Count > 0)
        {
            Report($"有人弱监督按当前 N.I.N.A. 设备逐项降级：{string.Join("；", warnings)}。这些项只 warning；已连接设备的明确危险状态仍阻断");
            return GateResult.Warn(
                "WEAK_SUPERVISION_ENVIRONMENT_CONNECTION_WARNING",
                $"Weak supervision continued with {string.Join("; ", warnings)}. This is not unattended authority.");
        }

        return GateResult.Pass(
            "NINA_ENVIRONMENT_ADAPTERS_CONNECTED",
            "The locked Safety Monitor, weather, dome/roll-off-roof and optical-cover adapters are connected; no roof or cover motion was issued by this connection step.");
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return GateResult.Pass(
                "WEAK_SUPERVISION_ROOF_OPERATOR_OWNED",
                "Weak supervision never issues an automatic roof-open command. A selected roof adapter is still read back, and an explicitly non-open roof blocks optical actions.");
        }

        if (!configuration.Environment.RequireOpenDomeOrRoof ||
            !configuration.Environment.Devices.HasDomeOrRoof)
        {
            return GateResult.Unknown(
                "UNATTENDED_ROOF_AUTHORITY_MISSING",
                "Full unattended mode requires a locked N.I.N.A. dome/roll-off-roof selection and live state authority.");
        }

        var dome = domeMediator.GetInfo();
        if (!dome.Connected)
        {
            return GateResult.Unknown("ROOF_STATE_UNKNOWN", "The locked N.I.N.A. roof adapter is disconnected.");
        }
        if (!EnvironmentAdapterIdentityMatches(dome.DeviceId, configuration.Environment.Devices.DomeOrRoofId))
        {
            return GateResult.Fail(
                "ROOF_ADAPTER_IDENTITY_MISMATCH",
                $"Connected roof adapter '{dome.DeviceId}' does not match locked selection '{configuration.Environment.Devices.DomeOrRoofId}'.");
        }
        if (dome.ShutterStatus == ShutterState.ShutterOpen)
        {
            ...
            Interlocked.Exchange(ref domeOrRoofLifecycleCommitted, 1);
            return GateResult.Pass(
                "UNATTENDED_ROOF_OPEN",
                "The locked N.I.N.A./RRCI roof adapter attests that the roll-off roof is open.");
        }
        if (Volatile.Read(ref domeOrRoofLifecycleCommitted) != 0)
        {
            return GateResult.Fail(
                "ROOF_CLOSED_AFTER_OPEN",
                $"This run already committed a roof-open lifecycle but the roof now reports {dome.ShutterStatus}. It will not be reopened inside the same run.");
        }
        if (!configuration.Environment.OpenDomeOrRoofOnStart)
        {
            return GateResult.Fail(
                "ROOF_NOT_OPEN",
                $"The roof reports {dome.ShutterStatus}, and automatic opening is disabled in the immutable run configuration.");
        }
        if (!dome.CanSetShutter)
        {
            return GateResult.Fail(
                "ROOF_OPEN_UNSUPPORTED",
                "The locked N.I.N.A./RRCI adapter does not report shutter/roll-off-roof command capability.");
        }
        if (dome.ShutterStatus is not ShutterState.ShutterClosed)
        {
            return GateResult.Unknown(
                "ROOF_NOT_READY_TO_OPEN",
                $"The roof must be stably closed before an unattended open command; current state is {dome.ShutterStatus}.");
        }

        await environmentMechanicalGate.WaitAsync(cancellationToken).ConfigureAwait(false);
        try
        {
            await CheckpointAndRejectStaleStageStackAsync(context, cancellationToken).ConfigureAwait(false);
            var prerequisite = ValidateRoofOpeningPrerequisites(context, plan);
            if (prerequisite.Disposition != GateDisposition.Passed) return prerequisite;

            var telescope = telescopeMediator.GetInfo();
            if (!telescope.Connected)
            {
                return GateResult.Unknown("ROOF_OPEN_MOUNT_DISCONNECTED", "The mount must be connected and parked before opening the roll-off roof.");
            }
            if (telescope.Slewing)
            {
                return GateResult.Unknown("ROOF_OPEN_MOUNT_MOVING", "The mount is moving; the roll-off roof will not be opened.");
            }
            if (!telescope.AtPark)
            {
                if (!telescope.CanPark)
                {
                    return GateResult.Fail("ROOF_OPEN_MOUNT_CANNOT_PARK", "The mount is not parked and does not report park capability.");
                }

                Report("全无人监管：开顶前先通过 N.I.N.A. 停放赤道仪");
                if (!await telescopeMediator.ParkTelescope(progress, cancellationToken).ConfigureAwait(false))
                {
                    return GateResult.Unknown("ROOF_OPEN_MOUNT_PARK_FAILED", "N.I.N.A. did not accept or complete the mount park required before roof opening.");
                }
                var parkGate = await WaitForMountParkedAsync(cancellationToken).ConfigureAwait(false);
                if (parkGate.Disposition != GateDisposition.Passed) return parkGate;
            }

            await CheckpointAndRejectStaleStageStackAsync(context, cancellationToken).ConfigureAwait(false);
            prerequisite = ValidateRoofOpeningPrerequisites(context, plan);
            if (prerequisite.Disposition != GateDisposition.Passed) return prerequisite;
            dome = domeMediator.GetInfo();
            if (!dome.Connected ||
                !EnvironmentAdapterIdentityMatches(dome.DeviceId, configuration.Environment.Devices.DomeOrRoofId) ||
                dome.ShutterStatus != ShutterState.ShutterClosed)
            {
                return GateResult.Unknown(
                    "ROOF_OPEN_STATE_CHANGED",
                    "The locked RRCI connection, identity or closed state changed immediately before the open command.");
            }

            Report("全无人监管：AIWeather、安全天气门和停放状态均通过，委托 N.I.N.A./RRCI 打开平移顶");
            // Arm the fail-safe before sending the command so an unsafe event
            // that arrives during the opening transition starts closure rather
            // than being mistaken for a still-closed pre-run state.
            Interlocked.Exchange(ref domeOrRoofOpenEstablished, 1);
            Interlocked.Exchange(ref domeOrRoofLifecycleCommitted, 1);
            if (!await domeMediator.OpenShutter(cancellationToken).ConfigureAwait(false))
            {
                if (domeMediator.GetInfo().ShutterStatus == ShutterState.ShutterClosed)
                {
                    Interlocked.Exchange(ref domeOrRoofOpenEstablished, 0);
                }
                return GateResult.Unknown(
                    "ROOF_OPEN_COMMAND_REJECTED",
                    "N.I.N.A./RRCI rejected the roof-open command. On an RRCI replica, verify fresh primary status and that replica commands plus replica opening are explicitly enabled.");
            }
            var openGate = await WaitForDomeOrRoofStateAsync(ShutterState.ShutterOpen, cancellationToken).ConfigureAwait(false);
            if (openGate.Disposition == GateDisposition.Passed)
            {
                Interlocked.Exchange(ref domeOrRoofOpenEstablished, 1);
            }
            else if (domeMediator.GetInfo().ShutterStatus == ShutterState.ShutterClosed)
            {
                Interlocked.Exchange(ref domeOrRoofOpenEstablished, 0);
            }
            await WriteAuditBestEffortAsync("unattended-roof-open", new
            {
                adapter = configuration.Environment.Devices.DomeOrRoofId,
                safetyMonitor = configuration.Environment.Devices.SafetyMonitorId,
                weather = configuration.Environment.Devices.WeatherDataId,
                openGate.Code,
                disposition = openGate.Disposition.ToString(),
                openGate.Message,
            }).ConfigureAwait(false);
            return openGate;
        }
        ...
        {
            return GateResult.Unknown("ROOF_OPEN_EXCEPTION", $"N.I.N.A./RRCI roof opening failed: {ex.Message}");
        }
        finally
        {
            environmentMechanicalGate.Release();
        }
    }

    private GateResult ValidateRoofOpeningPrerequisites(ObservationContext context, ObservationPlan plan)
    {
        if (Volatile.Read(ref environmentSafetyTrip) is { } trip)
        {
            return GateResult.Fail("ENVIRONMENT_SAFETY_TRIPPED", trip);
        }
        ...
        if (!configuration.MatchesCurrentProfile(
                ...
                profileService.ActiveProfile.ImageFileSettings.FilePattern,
                CaptureCurrentEnvironmentDeviceSelection(),
                out var currentSha256,
                out var differences))
        {
            return GateResult.Unknown(
                "REAL_PROFILE_DRIFT",
                $"The immutable action configuration changed before roof opening. Locked {configuration.ActionConfigurationSha256}, current {currentSha256}. Changed: {differences}.");
        }
        var owners = ValidateNinaProfileOwnerSelections(context.Plan);
        if (owners.Disposition != GateDisposition.Passed) return owners;
        ...
        var environment = ValidateEnvironment(
            plan with
            {
                PlannedStartUtc = DateTimeOffset.UtcNow,
                PlannedDuration = context.RemainingWorstCaseDuration ?? plan.PlannedDuration,
            },
            allowClosedRoofForUnattendedOpening: true);
        if (environment.Disposition != GateDisposition.Passed) return environment;
        return ValidateMountClock();
    }

    private async Task<GateResult> WaitForMountParkedAsync(CancellationToken cancellationToken)
    {
        var deadline = DateTimeOffset.UtcNow.AddSeconds(configuration.Environment.DomeOrRoofTransitionTimeoutSeconds);
        while (DateTimeOffset.UtcNow <= deadline)
        {
            ...
            if (!telescope.Connected)
            {
                return GateResult.Unknown("MOUNT_DISCONNECTED_WHILE_PARKING", "The mount disconnected while preparing the roof transition.");
            }
            if (telescope.AtPark && !telescope.Slewing)
            {
                return GateResult.Pass("MOUNT_PARKED_FOR_ROOF", "N.I.N.A. attested that the mount is parked and stopped.");
            }
            await Task.Delay(250, cancellationToken).ConfigureAwait(false);
        }
        return GateResult.Unknown(
            "MOUNT_PARK_TIMEOUT",
            $"The mount did not reach a checked parked state within {configuration.Environment.DomeOrRoofTransitionTimeoutSeconds} seconds.");
    }

    private async Task<GateResult> WaitForDomeOrRoofStateAsync(
        ShutterState expected,
        CancellationToken cancellationToken)
    {
        var deadline = DateTimeOffset.UtcNow.AddSeconds(configuration.Environment.DomeOrRoofTransitionTimeoutSeconds);
        while (DateTimeOffset.UtcNow <= deadline)
        {
            ...
            if (!dome.Connected)
            {
                return GateResult.Unknown("ROOF_DISCONNECTED_DURING_TRANSITION", "The RRCI roof adapter disconnected during a commanded transition.");
            }
            if (!EnvironmentAdapterIdentityMatches(dome.DeviceId, configuration.Environment.Devices.DomeOrRoofId))
            {
                return GateResult.Fail("ROOF_IDENTITY_CHANGED_DURING_TRANSITION", "The connected roof adapter identity changed during a commanded transition.");
            }
            if (dome.ShutterStatus == expected)
            {
                return GateResult.Pass(
                    expected == ShutterState.ShutterOpen ? "UNATTENDED_ROOF_OPEN" : "UNATTENDED_ROOF_CLOSED",
                    $"N.I.N.A./RRCI attested roll-off-roof state {expected}.");
            }
            if (dome.ShutterStatus == ShutterState.ShutterError)
            {
                return GateResult.Fail("ROOF_TRANSITION_ERROR", "The RRCI roof adapter entered ShutterError during the commanded transition.");
            }
            await Task.Delay(250, cancellationToken).ConfigureAwait(false);
        }
        return GateResult.Unknown(
            "ROOF_TRANSITION_TIMEOUT",
            $"The roof did not reach {expected} within {configuration.Environment.DomeOrRoofTransitionTimeoutSeconds} seconds.");
    }
    ...
    private GateResult ValidateNinaProfileOwnerSelections(ObservationPlan plan)
    {
        ...
        return NinaProfileOwnerPreflight.Validate(
            ...
            new NinaProfileOwnerExpectation(
                ...
                configuration.Environment.Devices.OpticalCoverId,
    ...
    private GateResult ValidateCurrentActionPrerequisites(ObservationContext context)
    {
                ...
                out var profileDifferences))
        {
            return GateResult.Unknown(
                "REAL_PROFILE_DRIFT",
                $"An action-bearing N.I.N.A. Profile value changed after this run was locked. Locked {configuration.ActionConfigurationSha256}, current {currentConfigurationSha256}. Changed: {profileDifferences}. No physical action is permitted.");
        }
        ...
        if (environment.Severity == GateSeverity.Warning)
        {
            return GateResult.Warn(
                "CURRENT_ACTION_PREREQUISITES_WITH_WARNING",
                $"Current real-mode authorization, immutable Profile, horizon and mount UTC passed immediately before the action. {environment.Message} Optical-cover availability is evaluated separately.",
                clock.Metrics);
        }
    ...
    {
        var selectedCoverId = configuration.Environment.Devices.OpticalCoverId;
        if (!configuration.Environment.Devices.HasOpticalCover)
        {
            return configuration.Environment.RequireOpenOpticalCover
                ? GateResult.Unknown(
                    "OPTICAL_COVER_SELECTION_MISSING",
                    "The locked unattended Profile has no optical-cover adapter selection.")
                : GateResult.Warn(
                    "WEAK_SUPERVISION_OPTICAL_COVER_NOT_SELECTED",
                    "No optical-cover adapter is selected; weak supervision records a warning and continues without commanding a cover.");
        }
        ...
        if (!configuration.Environment.RequireOpenOpticalCover)
        {
            try
            {
                ...
                if (!EnvironmentAdapterIdentityMatches(optionalCover.DeviceId, selectedCoverId))
                {
                    return GateResult.Warn(
                        "WEAK_SUPERVISION_OPTICAL_COVER_IDENTITY_UNVERIFIED",
                        $"The connected optical-cover adapter '{optionalCover.DeviceId}' does not match the locked selection '{selectedCoverId}'; weak supervision will not command or trust it.");
                }
        ...
        try
        {
            ...
            if (!EnvironmentAdapterIdentityMatches(cover.DeviceId, selectedCoverId))
            {
                return GateResult.Fail(
                    "OPTICAL_COVER_IDENTITY_MISMATCH",
                    $"Connected optical-cover adapter '{cover.DeviceId}' does not match locked selection '{selectedCoverId}'.");
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!configuration.Environment.Devices.HasOpticalCover)
        {
            return GateResult.Unknown("OPTICAL_COVER_SELECTION_MISSING", "No locked optical-cover adapter may receive an open command.");
        }
    ...
    private async Task<string?> CloseOpticalCoverAsync(string reason, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (!devices.HasOpticalCover)
            {
                if (configuration.Environment.WeakSupervisionEnabled)
                {
                    Report("有人弱监督：未选择镜盖设备，收尾跳过镜盖关闭并记录 warning");
                    return null;
                }
                return "No locked optical-cover adapter is selected; closed state cannot be attested.";
            }
            var info = flatDeviceMediator.GetInfo();
            if (!info.Connected)
            {
                // Reconnecting the exact locked adapter is read-only with
                // respect to the cover. Do this once before deciding cleanup
                // is impossible; the reconnect itself sends no cover command.
                // Only a fresh identity/state readback may reach the existing
                // idempotent close plus terminal-readback path. An ambiguous or
                // disconnected transition is never represented as success.
                var selection = ValidateCurrentEnvironmentDeviceSelections();
                if (selection.Disposition != GateDisposition.Passed)
                {
                    return $"{selection.Code}: {selection.Message}";
                }
                var accepted = await flatDeviceMediator.Connect().ConfigureAwait(false);
                info = flatDeviceMediator.GetInfo();
                await WriteAuditBestEffortAsync("optical-cover-finalize-reconnect", new
                {
                    reason,
                    adapter = devices.OpticalCoverId,
                    accepted,
                    connected = info.Connected,
                    observedDeviceId = info.DeviceId,
                    readOnlyReconnect = true,
                    coverCommandSent = false,
                }).ConfigureAwait(false);
                if (!accepted || !info.Connected)
                {
                    if (configuration.Environment.WeakSupervisionEnabled)
                    {
                        Report("有人弱监督：已选镜盖只读重连失败，收尾跳过镜盖命令并记录 warning");
                        return null;
                    }
                    return "Optical cover is disconnected after one exact-adapter read-only reconnect; closed state cannot be attested.";
                }
            }
            if (!EnvironmentAdapterIdentityMatches(info.DeviceId, devices.OpticalCoverId))
            {
                return $"Connected optical-cover adapter '{info.DeviceId}' does not match locked selection '{devices.OpticalCoverId}'.";
            }
        ...
        CancellationToken cancellationToken)
    {
        if (configuration.Environment.WeakSupervisionEnabled)
        {
            Report("有人弱监督：平移顶保持操作员所有，自动收尾不发关顶命令");
            return null;
        }
        if (!configuration.Environment.Devices.HasDomeOrRoof)
        {
            return "No locked dome/roll-off-roof adapter is selected; roof closure cannot be attested.";
        }
        ...
        try
        {
            ...
            if (!dome.Connected)
            {
                return "The locked RRCI roof adapter is disconnected; no roof command was sent.";
            }
            if (!EnvironmentAdapterIdentityMatches(dome.DeviceId, configuration.Environment.Devices.DomeOrRoofId))
            {
                return $"Connected roof adapter '{dome.DeviceId}' does not match locked selection '{configuration.Environment.Devices.DomeOrRoofId}'; no roof command was sent.";
            }
            if (dome.ShutterStatus == ShutterState.ShutterClosed)
            {
                Interlocked.Exchange(ref domeOrRoofOpenEstablished, 0);
                var closedRoofMount = telescopeMediator.GetInfo();
                return closedRoofMount.Connected && closedRoofMount.AtPark && !closedRoofMount.Slewing
                    ? null
                    : "The roll-off roof already reports closed, but the mount is not attested connected, parked and stopped. " +
                      "No mount motion was attempted under a closed roof; terminal cleanup is incomplete.";
            }
            if (!dome.CanSetShutter)
            {
                return "The locked RRCI adapter cannot command the roll-off roof closed.";
            }
            if (dome.ShutterStatus == ShutterState.ShutterError)
            {
                return "The locked RRCI adapter reports ShutterError; automatic closure cannot be attested.";
            }

            var telescope = telescopeMediator.GetInfo();
            if (!telescope.Connected)
            {
                return "The mount is disconnected, so its parked state cannot be attested and the roll-off roof will not be commanded closed.";
            }
            if (telescope.Slewing)
            {
                try { telescopeMediator.StopSlew(); }
                catch (Exception ex) { return $"Mount slew could not be stopped before roof closure: {ex.Message}"; }
            }
            telescope = telescopeMediator.GetInfo();
            if (!telescope.AtPark)
            {
                if (!telescope.CanPark)
                {
                    return "The mount is not parked and does not report park capability; the roll-off roof will not be commanded closed.";
                }
                Report("全无人监管收尾：镜盖已处理，正在通过 N.I.N.A. 停放赤道仪");
                if (!await telescopeMediator.ParkTelescope(progress, cancellationToken).ConfigureAwait(false))
                {
                    return "N.I.N.A. did not accept or complete the mount park required before roof closure.";
                }
                var parkGate = await WaitForMountParkedAsync(cancellationToken).ConfigureAwait(false);
                if (parkGate.Disposition != GateDisposition.Passed)
                {
                    return $"{parkGate.Code}: {parkGate.Message}";
                }
            }
            ...
            if (!dome.Connected ||
                !EnvironmentAdapterIdentityMatches(dome.DeviceId, configuration.Environment.Devices.DomeOrRoofId))
            {
                return "The locked RRCI connection or identity changed after mount park; no roof command was sent.";
            }
            if (dome.ShutterStatus == ShutterState.ShutterClosed)
            {
                Interlocked.Exchange(ref domeOrRoofOpenEstablished, 0);
            ...
            Report("全无人监管收尾：赤道仪已停放，委托 N.I.N.A./RRCI 关闭平移顶");
            if (!await domeMediator.CloseShutter(cancellationToken).ConfigureAwait(false))
            {
                return "N.I.N.A./RRCI rejected the roof-close command. On an RRCI replica, verify fresh primary status, replica command authority and required-node parked heartbeats.";
            }
            var closeGate = await WaitForDomeOrRoofStateAsync(ShutterState.ShutterClosed, cancellationToken).ConfigureAwait(false);
            if (closeGate.Disposition == GateDisposition.Passed)
            {
                Interlocked.Exchange(ref domeOrRoofOpenEstablished, 0);
            }
            await WriteAuditBestEffortAsync("unattended-roof-close", new
            {
                reason,
                adapter = configuration.Environment.Devices.DomeOrRoofId,
                closeGate.Code,
                disposition = closeGate.Disposition.ToString(),
                closeGate.Message,
                mountParked = telescopeMediator.GetInfo().AtPark,
            }).ConfigureAwait(false);
            return closeGate.Disposition == GateDisposition.Passed
                ? null
                : $"{closeGate.Code}: {closeGate.Message}";
        }
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        {
            throw;
        ...
        {
            return $"Mount park / roll-off-roof close failed: {ex.Message}";
        }
        finally
        {
            environmentMechanicalGate.Release();
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Starting a real observation explicitly authorizes this one bounded connection attempt.
        // Runtime revalidation calls EvaluateInterlocksAsync with connectUvex=false so an operator
        // disconnect or maintenance takeover is never silently undone in the background.
        await CheckpointAndRejectStaleStageStackAsync(context, cancellationToken).ConfigureAwait(false);
        await using var lease = await uvex.AcquireLeaseAsync(
            $"nina-auto-night-setup-{context.Plan.ObservationRunId}",
            cancellationToken).ConfigureAwait(false);
        var connected = await lease.ConnectAndVerifyAsync(cancellationToken).ConfigureAwait(false);
        await WriteAuditBestEffortAsync("uvex-night-setup-auto-connected", new
        {
            context.Plan.ObservationRunId,
            connected.PortName,
            connectionState = connected.ConnectionState.ToString(),
            connected.PositionKnown,
            positionTrust = connected.PositionTrust.ToString(),
            connected.SlitPosition,
            connected.GratingPositionSteps,
            connected.FocusPositionSteps,
        }).ConfigureAwait(false);
        return connected;
    }
    ...
    private GateResult ValidateEnvironment(
        ObservationPlan plan,
        bool allowClosedRoofForUnattendedOpening = false)
    {
        ...
        var environmentWarnings = new List<string>();
        ...
        if (!devices.HasSafetyMonitor)
        {
            if (configuration.Environment.RequireSafetyMonitor)
            {
                return GateResult.Unknown("SAFETY_MONITOR_SELECTION_MISSING", "A selected and connected Safety Monitor is required for this unattended REAL observation.");
            }
            environmentWarnings.Add("Safety Monitor not selected");
        }
        else
        {
            ...
            if (!safety.Connected)
            {
                if (configuration.Environment.RequireSafetyMonitor)
                {
                    return GateResult.Unknown("SAFETY_MONITOR_MISSING", "The locked Safety Monitor is not connected for this unattended REAL observation.");
                }
                environmentWarnings.Add("selected Safety Monitor unavailable");
            }
            else if (!EnvironmentAdapterIdentityMatches(safety.DeviceId, devices.SafetyMonitorId))
            {
                if (configuration.Environment.RequireSafetyMonitor)
                {
                    return GateResult.Fail("SAFETY_MONITOR_IDENTITY_MISMATCH", $"Connected Safety Monitor '{safety.DeviceId}' does not match locked selection '{devices.SafetyMonitorId}'.");
                }
                environmentWarnings.Add("selected Safety Monitor identity could not be trusted");
            }
        ...
        if (!devices.HasDomeOrRoof)
        {
            if (configuration.Environment.RequireOpenDomeOrRoof)
            {
                return GateResult.Unknown("ROOF_ADAPTER_SELECTION_MISSING", "A selected and connected dome/roll-off-roof adapter is required for this unattended REAL observation.");
            }
            environmentWarnings.Add("dome/roll-off-roof adapter not selected");
        }
        else
        {
            var dome = domeMediator.GetInfo();
            if (!dome.Connected)
            {
                if (configuration.Environment.RequireOpenDomeOrRoof)
                {
                    return GateResult.Unknown("ROOF_STATE_UNKNOWN", "The locked dome/roll-off-roof adapter is disconnected; roof state will not be guessed.");
                }
                environmentWarnings.Add("selected dome/roll-off-roof adapter unavailable");
            }
            else if (!EnvironmentAdapterIdentityMatches(dome.DeviceId, devices.DomeOrRoofId))
            {
                if (configuration.Environment.RequireOpenDomeOrRoof)
                {
                    return GateResult.Fail("ROOF_ADAPTER_IDENTITY_MISMATCH", $"Connected roof adapter '{dome.DeviceId}' does not match locked selection '{devices.DomeOrRoofId}'.");
                }
                environmentWarnings.Add("selected dome/roll-off-roof identity could not be trusted");
            }
            else if (dome.ShutterStatus != ShutterState.ShutterOpen &&
                     !(allowClosedRoofForUnattendedOpening &&
                       !configuration.Environment.WeakSupervisionEnabled &&
                       dome.ShutterStatus == ShutterState.ShutterClosed))
            {
                return GateResult.Fail("ROOF_NOT_OPEN", $"Dome/roll-off-roof state is {dome.ShutterStatus}, not open.");
            }
        }
        ...
        if (!devices.HasWeatherData)
        {
            if (configuration.Environment.RequireWeatherData)
            {
                return GateResult.Unknown("WEATHER_ADAPTER_SELECTION_MISSING", "A selected and connected weather adapter is required for this unattended REAL observation.");
            }
            environmentWarnings.Add("weather adapter not selected");
        }
        else
        {
            ...
            if (!weather.Connected)
            {
                if (configuration.Environment.RequireWeatherData)
                {
                    return GateResult.Unknown("WEATHER_STATE_UNKNOWN", "The locked weather adapter is disconnected; weather will not be guessed.");
                }
                environmentWarnings.Add("selected weather adapter unavailable");
            }
            else if (!EnvironmentAdapterIdentityMatches(weather.DeviceId, devices.WeatherDataId))
            {
                if (configuration.Environment.RequireWeatherData)
                {
                    return GateResult.Fail("WEATHER_ADAPTER_IDENTITY_MISMATCH", $"Connected weather adapter '{weather.DeviceId}' does not match locked selection '{devices.WeatherDataId}'.");
                }
                environmentWarnings.Add("selected weather-adapter identity could not be trusted");
            }
            else
            {
            ...
            if (missing.Count > 0)
            {
                ...
                environmentWarnings.Add($"weather metrics missing: {string.Join(", ", missing)}");
            }
            ...
            if (double.IsFinite(weather.Humidity) && weather.Humidity > configuration.Environment.MaximumHumidityPercent)
            {
                environmentWarnings.Add($"high humidity {weather.Humidity:F1}% exceeds advisory threshold {configuration.Environment.MaximumHumidityPercent:F1}%");
            }
        ...
        var horizon = HorizonCalculator.Evaluate(plan with { PlannedStartUtc = DateTimeOffset.UtcNow }).ToGateResult();
        if (horizon.Disposition != GateDisposition.Passed) return horizon;
        return environmentWarnings.Count == 0
            ? horizon
            : GateResult.Warn(
                configuration.Environment.WeakSupervisionEnabled
                    ? "WEAK_SUPERVISION_ENVIRONMENT_WARNING"
                    : "ENVIRONMENT_ADVISORY_WARNING",
                configuration.Environment.WeakSupervisionEnabled
                    ? $"Operator weak supervision is active; {string.Join("; ", environmentWarnings)}. " +
                      "The run is not unattended. Connected adapters that explicitly report danger still block actions."
                    : $"Environmental advisory: {string.Join("; ", environmentWarnings)}. " +
                      "Humidity alone is not evidence of rain or an unsafe roof; acquisition may continue.",
                horizon.Metrics);
    }

    private static bool EnvironmentAdapterIdentityMatches(string? actual, string expected) =>
        ...
        CancellationToken cancellationToken)
    {
        ...
        await CheckpointAndRejectStaleStageStackAsync(context, cancellationToken).ConfigureAwait(false);
        await RequireImmediatePhysicalActionGatesAsync(context, cancellationToken).ConfigureAwait(false);
        Report("赤道仪未跟踪；通过 N.I.N.A. 单次启用恒星时跟踪，等待驱动回读确认");
        ...
        while (accepted &&
               verified.Connected &&
               !verified.AtPark &&
               !verified.TrackingEnabled)
        {
            cancellationToken.ThrowIfCancellationRequested();
            var elapsed = DateTimeOffset.UtcNow - requestedUtc;
            var remaining = MountTrackingEnableTimeout - elapsed;
            if (remaining <= TimeSpan.Zero) break;

            await Task.Delay(
                    remaining < MountTrackingEnablePollInterval
                        ? remaining
                        : MountTrackingEnablePollInterval,
                    cancellationToken)
                .ConfigureAwait(false);
            verified = telescopeMediator.GetInfo();
            pollCount++;
        }

        var elapsedMilliseconds = Math.Max(0d, (DateTimeOffset.UtcNow - requestedUtc).TotalMilliseconds);
        await WriteAuditBestEffortAsync("telescope-tracking-enable", new
        {
            ...
            pollCount,
            elapsedMilliseconds,
            timeoutSeconds = MountTrackingEnableTimeout.TotalSeconds,
            connected = verified.Connected,
            atPark = verified.AtPark,
            slewing = verified.Slewing,
            sideOfPier = verified.SideOfPier.ToString(),
        }).ConfigureAwait(false);

        if (!accepted)
        {
            return GateResult.Unknown(
                "TELESCOPE_TRACKING_ENABLE_REJECTED",
                "N.I.N.A. rejected the single tracking-enable request; no slew was started and the command was not repeated.");
        }
        if (!verified.Connected)
        {
            return GateResult.Unknown(
                "TELESCOPE_TRACKING_CONNECTION_LOST",
                "The telescope disconnected while N.I.N.A. was waiting for tracking confirmation; no slew was started.");
        }
        if (verified.AtPark)
        {
            return GateResult.Unknown(
                "TELESCOPE_TRACKING_ENABLE_PARKED",
                "N.I.N.A. still reports the mount parked after accepting tracking enable; no slew was started.");
        }
        if (!verified.TrackingEnabled)
        {
            return GateResult.Unknown(
                "TELESCOPE_TRACKING_ENABLE_TIMEOUT",
                $"N.I.N.A. accepted one tracking-enable request but did not report tracking within {MountTrackingEnableTimeout.TotalSeconds:F0} s. This is no longer treated as an immediate-readback failure; inspect a meridian/limit refusal or home the mount. No slew was started.");
        }

        return GateResult.Pass(
            "TELESCOPE_TRACKING_ENABLED",
            $"N.I.N.A. enabled and verified mount tracking after {elapsedMilliseconds / 1000d:F2} s ({pollCount} polls, one command).");
    }

    private async Task<StageResult> AcquireOptionalQhyWideFieldAsync(
        ObservationContext context,
        CancellationToken cancellationToken)
    {
        if (!qhyAvailableForRun)
        {
            return Warning(
                "QHY_WIDE_FIELD_SKIPPED",
                $"Optional QHY wide-field witness is unavailable ({qhyUnavailableReason ?? "reason unavailable"}); fresh G3 WCS remains the acquisition and motion authority.");
        }

        try
        {
            var result = await AcquireQhyWideFieldAsync(context, cancellationToken).ConfigureAwait(false);
            if (result.CanAdvance) return result;
            DisableQhyForRun($"{result.Gate.Code}: {result.Gate.Message}");
            return Warning(
                "QHY_WIDE_FIELD_DEGRADED_TO_G3",
                $"Optional QHY wide-field acquisition did not complete ({qhyUnavailableReason}). The run is continuing through fresh G3 WCS without QHY-derived motion.",
                result.Gate.Metrics,
                result.Metadata);
        }
        catch (OperationCanceledException) { throw; }
        catch (PhysicalActionGateException) { throw; }
        catch (Exception ex)
        {
            DisableQhyForRun($"QHY wide-field acquisition raised {ex.GetType().Name}: {ex.Message}");
            return Warning(
                "QHY_WIDE_FIELD_EXCEPTION_DEGRADED_TO_G3",
                $"{qhyUnavailableReason} Fresh G3 WCS remains available, so the optional QHY failure does not stop spectroscopy.");
        }
    }
    ...
    private async Task<StageResult> AcquireQhyWideFieldAsync(ObservationContext context, CancellationToken cancellationToken)
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(accepted.Sha256) || accepted.Metrics.QualityFlags.Count != 0)
            {
                return Attention(
                    ObservationStage.AcquireQhyWideField,
                    "QHY_FRAME_PROVENANCE_OR_QUALITY_INVALID",
                    $"QHY accepted frame cannot be used as a formal PL3 witness " +
                    $"(qualityFlags={string.Join(',', accepted.Metrics.QualityFlags)}, sha256Present={!string.IsNullOrWhiteSpace(accepted.Sha256)}).");
            }

            if (nightSetup is not null && IsGs350LockedNoMove(nightSetup.Value))
            {
                Report(
                    $"QHY {accepted.Settings.ExposureSeconds:G4}s 原始 FITS 已被 N.I.N.A./PL3 正式解出；GS350 为 Night Setup 锁定的保持位置，" +
                    "本次不伪造 FWHM、不移动 ToupTek AAF，直接把正式 PL3 解作为无运动广域见证并继续 fresh G3 路线");
                await WriteAuditBestEffortAsync("qhy-pl3-no-move-focus-deferred", new
                {
                    acceptedFrameId = accepted.FrameId,
                    accepted.FitsPath,
                    accepted.Sha256,
                    accepted.Settings.ExposureSeconds,
                    accepted.Metrics.DetectedStars,
                    accepted.Metrics.MedianFwhmPixels,
                    accepted.Metrics.QualityFlags,
                    solver = lastQhySolve.SolverIdentity,
                    solveSucceeded = lastQhySolve.Result.Success,
                    gs350MovementAuthorized = false,
                    qhyMountCorrectionAuthorized = false,
                    nextRouteOwner = "PHD2/G3",
                }).ConfigureAwait(false);
            }
            else
            {
                ...
                {
                    Report(
                        $"QHY {accepted.Settings.ExposureSeconds:G4}s 帧已被 N.I.N.A./PL3 解出，但可移动的 GS350 调焦路线尚未取得 FWHM；保留该帧并自动尝试曝光阶梯下一档 {nextExposure:G4}s");
                    qhyAcquisitionMinimumExposureSeconds = nextExposure;
                    qhyAcquisitionJobId = null;
                    lastQhyAcquisition = null;
                    lastQhySolve = null;
                ...
                return Attention(
                    ...
                    $"QHY {accepted.Settings.ExposureSeconds:G4}s accepted frame has no finite stellar FWHM after the configured exposure ladder was exhausted " +
                    $"(detectedStars={accepted.Metrics.DetectedStars}, qualityFlags={string.Join(',', accepted.Metrics.QualityFlags)}, sha256Present={!string.IsNullOrWhiteSpace(accepted.Sha256)}). " +
                    "This Night Setup permits GS350/AAF motion, so a measured live focus metric is required before movement.");
            }
        }
        ...
        if (accepted.Metrics.MedianFwhmPixels is { } fwhm && double.IsFinite(fwhm) && fwhm > 0)
        {
            metrics["gs350MedianFwhmPixels"] = fwhm;
            metrics["gs350MedianEllipticity"] = accepted.Metrics.MedianEllipticity ?? 0;
        }
        var focusEvidenceMode = currentQhyFocusMetric is null
            ? "LockedNoMovePl3Witness"
            : "LiveQhyFwhm";
        return Passed(
            "QHY_WIDE_FIELD_SOLVED",
            currentQhyFocusMetric is null
                ? $"QHY accepted frame {accepted.FrameId:D} formally solved with {residual:F1} arcsec target residual; GS350 remains at its locked no-move position and numeric FWHM is explicitly deferred."
            ...
            new Dictionary<string, string>
            {
                ...
                ["solver"] = lastQhySolve.SolverIdentity,
                ["gs350FocusEvidenceMode"] = focusEvidenceMode,
                ["qhyMountMotionAuthority"] = "false",
            });
    }
    ...
    private async Task<StageResult> CoarseCenterAsync(ObservationContext context, CancellationToken cancellationToken)
    {
        ...
        {
            if (UseOnSkyValidatedG3FirstAcquisition && !qhyAvailableForRun)
            {
                return Warning(
                    "QHY_COARSE_CENTER_SKIPPED_G3_OWNS_ACQUISITION",
                    $"No optional QHY witness is available ({qhyUnavailableReason ?? "reason unavailable"}). No QHY-derived motion was attempted; fresh G3 WCS owns large correction and bounded neighbouring-field recovery.");
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        currentQhyFocusMetric = null;
        qhyAcquisitionAttempt++;
        var reacquired = await AcquireOptionalQhyWideFieldAsync(context, cancellationToken).ConfigureAwait(false);
        ...
        bool allowChargedCurrentPositionHandoff = false)
    {
        ...
        {
            var placementPreset = commissioning?.Value.Phd2SlitPlacement;
            var coarseResidualPixels = lastG3Field.TargetIdentification.Target is { } target &&
                                       lastG3Field.SlitDetection.Gate.Disposition == GateDisposition.Passed
                ? PixelDistance(target.Centroid, lastG3Field.SlitDetection.Geometry.AcquisitionPoint)
                : double.NaN;
                ...
                coarseResidualPixels + lastG3Field.TargetIdentification.CatalogPositionSpreadPixels, placementPreset.CoarseHandoffResidualPixels))
            {
                ...
                // correction all the way to the slit window, reserving PHD2
                // lock shifts for the final small residual.
                return await RunG3WcsCenteringAsync(
                    context,
                    lastG3Field,
                    transferEvidencePath,
                    cancellationToken,
                    allowChargedCurrentPositionHandoff).ConfigureAwait(false);
            }
    ...
    {
        // This is the single success boundary between G3 acquisition and the
        // PHD2 placement stage.  Publish the exact immutable, mount-bound field
        // here so no successful recovery/WCS/search exit can advance while
        // leaving the following stage with a stale or null target identity.
        if (NeedsG3CoarseCentering(field))
    ...
    {
        // Establish the detector-fixed slit geometry before acquiring the first
        // target/PL3 frame.  The previous order found the target first and then
        // inserted two nine-frame HDR sequences, leaving an avoidable interval
        // in which an unguided target could drift before placement.  Once this
        // run-scoped cache exists, mount corrections refresh only the sky frame.
        var slitGeometryPreparation = await CaptureAndAnalyzeG3Async(
            context,
            cancellationToken,
            slitGeometryOnly: true).ConfigureAwait(false);
        if (slitGeometryPreparation.Gate.Disposition != GateDisposition.Passed)
        {
        ...
        if (probe.Gate.Disposition != GateDisposition.Passed)
        {
                ...
                (motionPrediction is not null && probe.Gate.Code == "G3_CLOUD_OR_TRANSPARENCY_INVALID"))
            {
                ...
                // the target can be invisible in G3, proceeds using the already
                // verified run-scoped slit geometry. This does not assert clear sky:
        ...
        G3WcsMotionPrediction? motionPrediction = null)
    {
        ...
        for (var index = 0; index < preset.ExposureMilliseconds.Count; index++)
        {
            ...
            try
            {
                captured = await CaptureG3FullFrameForAcquisitionAsync(
            ...
            {
                var transientGate = ClassifyG3PlateSolveTransientFailure(
                    G3PlateSolveTransientOperation.Capture,
                    ex);
                if (transientGate is null) throw;
                latest = await RecordG3PlateSolveTransientFailureAsync(
            ...
            try
            {
                // Computing SHA-256 is mandatory before a frame can be bound or
                // promoted. A temporary source-file read failure invalidates
                // only this disposable path; cryptographic, access, identity
                // and cancellation failures are not classified as transient.
                sha256 = await ComputeFileSha256Async(captured.Path, cancellationToken).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                var transientGate = ClassifyG3PlateSolveTransientFailure(
            ...
            try
            {
                image = await imageDataFactory.CreateFromFile(
                    captured.Path,
                    16,
                    false,
            ...
            if (!content.HasCoherentSource)
            {
                ...
                // classifier.  It is not a pre-solver authority: PL3 has
                // already proved that it can solve sparse fields which this
                // local morphology heuristic does not recognize.  Always give
                // a valid immutable FITS to the configured N.I.N.A. solver;
                // only when the formal solve also fails may a zero-structure
                // assessment prohibit neighbouring-field motion.
                PublishG3Preview(
                    image,
            ...
            var attemptPath = await PublishRunJsonEvidenceAsync(
                ...
                new
                {
                    ...
                    contentDisposition = content.Gate.Disposition.ToString(),
                    ...
                    solvedRaDegrees = solve.Result.Coordinates?.RADegrees,
                    solvedDecDegrees = solve.Result.Coordinates?.Dec,
                    solveHintAuthority = solveHint.Authority,
                    solveHintReason = solveHint.Reason,
                    solveHintRaDegrees = solveHint.Coordinates.RADegrees,
                    solveHintDecDegrees = solveHint.Coordinates.Dec,
                    projectedTarget,
                    mountBinding = probeMountBinding,
                },
                captured.Path,
        ...
        // failed formal solve and must not authorize neighbouring-field motion.
        // Only an ordinary completed solve attempt may provide the existing
        // structured-field recovery authority.
        var hasStructuredContent = attempts.Any(attempt =>
            attempt.CoherentSourceCount > 0 &&
        ...
        var summarySourcePath = attempts
            ...
            ?.FramePath;
        if (string.IsNullOrWhiteSpace(summarySourcePath) || !File.Exists(summarySourcePath))
        {
        ...
        var summaryPath = await PublishRunJsonEvidenceAsync(
            ...
            new
            {
                ...
                mountMotionAuthorized = boundedSparseRecoveryAuthorized,
            },
            summarySourcePath,
            cancellationToken).ConfigureAwait(false);
        ...
        G3FieldState? freshMountBoundHandoffAuthority = null)
    {
        ...
        if (durableG3AcquisitionMotion is { } existing)
        {
            ...
            var inheritedOriginRadius = G3AcquisitionMotionPlanner.AngularSeparationArcseconds(
                ...
                NormalizeDegrees(origin.RADegrees),
                origin.Dec);
            var handoffToleranceArcseconds = Math.Max(
                existing.ArrivalToleranceArcseconds,
                MountMotionFamilyHandoffToleranceArcseconds);
            var currentReadbackSeparationArcseconds = G3AcquisitionMotionPlanner.AngularSeparationArcseconds(
                existing.PriorReportedRaDegrees,
                existing.PriorReportedDeclinationDegrees,
                NormalizeDegrees(origin.RADegrees),
                origin.Dec);
        ...
        Phd2DependencyRebuildStopProof? stoppedGuideProof = null)
    {
        ...
        var stableNearOriginToleranceArcseconds =
            G3AcquisitionMotionPlanner.ComputeStableNearOriginToleranceArcseconds(
                state,
                configuration.G3.WcsFreshSolveAuthorizationResidualArcseconds);
        if (!double.IsFinite(stableNearOriginToleranceArcseconds) ||
            stableNearOriginToleranceArcseconds < MountCommandArrivalToleranceArcseconds)
        {
            return new G3AcquisitionMotionReturnResult(
                false,
                state,
                path,
                "The stable near-origin return tolerance is invalid; no automatic return was attempted.");
        }
        ...
        while (true)
        {
            ...
            // A driver can finish a long segmented return a fraction of an
            // arcsecond away from the exact saved coordinate.  If the strict
            // action budget is already consumed, do not manufacture one more
            // mount action for that harmless sub-arcsecond readback.  Prove
            // that the mount is stable, unchanged in epoch/pier side and still
            // inside the separately bounded near-origin envelope, then settle
            // the durable ledger without issuing another command.
            if (!stableNearOriginReadbackAccepted &&
                state.CurrentRadiusArcseconds > MountCommandArrivalToleranceArcseconds &&
                state.CurrentRadiusArcseconds <= stableNearOriginToleranceArcseconds)
            {
                var originCoordinate = new Coordinates(
                    state.OriginRaDegrees,
                    state.OriginDeclinationDegrees,
                    reported.Epoch,
                    Coordinates.RAType.Degrees);
                var stableNearOrigin = await WaitForG3PostSlewStabilityAsync(
                    context,
                    originCoordinate,
                    reported,
                    state.PierSide,
                    state.CoordinateEpoch,
                    "durable G3 no-motion near-origin verification",
                    stableNearOriginToleranceArcseconds,
                    cancellationToken).ConfigureAwait(false);
                if (stableNearOrigin.Gate.Disposition != GateDisposition.Passed ||
                    stableNearOrigin.Reported is null)
                {
                    return new G3AcquisitionMotionReturnResult(
                        false,
                        state,
                        path,
                        $"{stableNearOrigin.Gate.Code}: {stableNearOrigin.Gate.Message}");
                }

                state = ReanchorG3AcquisitionMotionFromReportedPosition(state, stableNearOrigin.Reported) with
                {
                    UpdatedUtc = DateTimeOffset.UtcNow,
                    LastReason = $"No-motion return verification remained stable at {state.CurrentRadiusArcseconds:F3} arcsec from origin; the {stableNearOriginToleranceArcseconds:F2} arcsec near-origin settlement envelope is authorized without consuming another action.",
                };
            ...
            if (step.AlreadyAtOrigin)
            {
                state = state with
                {
                    ...
                    PriorReportedRaDegrees = NormalizeDegrees(reported.RADegrees),
                    PriorReportedDeclinationDegrees = reported.Dec,
                    CommandedRaDegrees = NormalizeDegrees(reported.RADegrees),
                    CommandedDeclinationDegrees = reported.Dec,
                    CommandMagnitudeArcseconds = 0,
                    UpdatedUtc = DateTimeOffset.UtcNow,
                    LastReason = stableNearOriginReadbackAccepted
                        ? $"The durable G3 acquisition return settled without another command at the stable reported position {state.CurrentRadiusArcseconds:F2} arcsec from its saved origin; the real offset remains charged and no budget was reset."
                        : $"The durable G3 acquisition return reached a reported position {state.CurrentRadiusArcseconds:F2} arcsec from its saved origin; the real readback remains charged and no budget was reset.",
                };
                await PersistG3AcquisitionMotionAsync(state, CancellationToken.None).ConfigureAwait(false);
            ...
            if (!double.IsFinite(stability.ReportedDriftArcseconds) ||
                stability.ReportedDriftArcseconds > MountCommandArrivalToleranceArcseconds ||
                !double.IsFinite(stability.CommandResidualArcseconds))
            {
                return new G3AcquisitionMotionReturnResult(
                    false,
            ...
            // Keep the durable obligation and let the existing planner consume
            // another bounded correction; its cumulative, attempt and elapsed
            // limits prevent an unbounded retry loop.
            stableNearOriginReadbackAccepted = false;
        }
    }
            ...
            foreach (var item in discovered)
            {
                ...
                if (item.State!.Phase == G3AcquisitionMotionPhase.SettledBudgetLedger &&
                    !string.Equals(
                        ...
                        StringComparison.Ordinal))
                {
                    // A settled ledger proves that the mount reached the
                    // recorded origin and has no outstanding recovery action.
                    // A later explicit observation must not inherit that old
                    // run's consumed counters or wall-clock merely because the
                    // old manifest ended paused rather than terminal.  Doing so
                    // made a fresh run immediately exhaust its acquisition
                    // time, return a valid PL3 correction to the origin, and
                    // then attempt an undersized neighbour search.  Preserve
                    // the immutable historical ledger, but close it for
                    // cross-run recovery exactly as we already do for settled
                    // PHD2 lock ledgers.
                    terminalSettled.Add(item);
                    continue;
                }
        ...
        bool allowChargedCurrentPositionHandoff = false)
    {
        ...
        if (commissioning?.Value.Phd2SlitPlacement is not { } placementPreset)
        {
            return Attention(
                ObservationStage.AcquireG3SlitField,
                "G3_WCS_SLIT_DESTINATION_UNCOMMISSIONED",
                "The commissioned PHD2 acquisition residual and slit destination are required before WCS-derived slit acquisition.");
        }
        var limitIssues = limits.Validate();
        if (limitIssues.Count > 0)
        {
               ...
               currentField.Solve.Result.Coordinates is not null)
        {
            ...
            if (currentField.Image is null)
            {
                stopReason = "The fresh solved G3 field has no immutable image dimensions for detector-to-slit inversion.";
                break;
            }
            var targetCoordinates = TargetCoordinates(context.Plan);
            var desiredTargetPixel = currentField.SlitDetection.Gate.Disposition == GateDisposition.Passed
                ? currentField.SlitDetection.Geometry.AcquisitionPoint
                : commissioning.SlitGeometry.AcquisitionPoint;
            var projectionDestination = G3CatalogTargetPositionPolicy.ProjectionDestination(
                currentField.TargetIdentification, desiredTargetPixel, placementPreset.TargetSearchRadiusPixels);
            G3WcsInverseSolution inverse;
            try
            {
                inverse = G3WcsTargetProjector.SolveCenterForTargetAtPixel(
                    targetCoordinates,
                    currentField.Solve.Result,
            ...
            {
                stopReason = $"The fresh G3 WCS could not be inverted onto the detector-fixed slit destination: {ex.Message}";
                break;
            }
                ...
                targetToSlitResidualPixels + currentField.TargetIdentification.CatalogPositionSpreadPixels <= placementPreset.CoarseHandoffResidualPixels)
            {
                ...
                // The following PlaceTargetOnSlit stage consumes lastG3Field, not
                // the StageResult metadata, so publish the exact field that proved
                // coarse arrival before advancing. Leaving the pre-centering field
                // here loses the quality-gated target identity at the stage boundary.
                lastG3Field = currentField;
            ...
            // Compare total remaining travel before/after the move. A neighbour
            // step is only part of that travel; comparing the next total with
            // that short step can reject a genuinely improving fresh WCS.
            var priorDirectCorrection = G3AcquisitionMotionPlanner.SignedTangentOffsetArcseconds(
                NormalizeDegrees(currentField.Solve.Result.Coordinates.RADegrees),
                currentField.Solve.Result.Coordinates.Dec,
                NormalizeDegrees(inverse.DesiredG3Center.RADegrees), inverse.DesiredG3Center.Dec);
            var priorRequiredMotionArcseconds = Math.Sqrt(
                priorDirectCorrection.RaArcseconds * priorDirectCorrection.RaArcseconds +
                priorDirectCorrection.DecArcseconds * priorDirectCorrection.DecArcseconds);
            var neighbourArrivalTolerancePixels = state.ArrivalToleranceArcseconds / currentField.Solve.Result.Pixscale;
            ...
            var targetCorrection = G3AcquisitionMotionPlanner.SignedTangentOffsetArcseconds(
                NormalizeDegrees(currentField.Solve.Result.Coordinates.RADegrees),
                currentField.Solve.Result.Coordinates.Dec,
                NormalizeDegrees(inverse.DesiredG3Center.RADegrees),
                inverse.DesiredG3Center.Dec);
            ...
            {
                stopReason = "The fresh solved-centre-to-slit-destination correction is invalid.";
                break;
            }
            ...
            var expectedCommandedG3Center = G3AcquisitionMotionPlanner.ApplyTangentOffsetArcseconds(
                NormalizeDegrees(currentField.Solve.Result.Coordinates.RADegrees),
                currentField.Solve.Result.Coordinates.Dec,
            ...
            try
            {
                ...
                var estimatedArrivalSolve = new PlateSolveResult
                {
                    ...
                    PositionAngle = currentField.Solve.Result.PositionAngle,
                    Flipped = currentField.Solve.Result.Flipped,
                };
                var predictedTarget = G3WcsTargetProjector.Project(
                    targetCoordinates,
                    estimatedArrivalSolve,
                    currentField.Image.Properties.Width,
                    currentField.Image.Properties.Height,
                    currentField.Solve.SolverIdentity);
                if (currentField.TargetIdentification.HasCatalogPositionRefinement)
                {
                    ...
                    // point if the immediate arrival frame cannot solve.
                    predictedTarget = new PixelPoint(
                        predictedTarget.X - (projectionDestination.X - desiredTargetPixel.X),
                        predictedTarget.Y - (projectionDestination.Y - desiredTargetPixel.Y));
                }
                motionPrediction = new G3WcsMotionPrediction(
                    predictedTarget,
                    MaximumUncertaintyPixels: MountCommandArrivalToleranceArcseconds / currentField.Solve.Result.Pixscale + inverse.InverseResidualPixels,
                    currentField.FramePath,
                    currentField.Solve.EvidencePath,
                    currentField.MountBinding?.BindingSha256 ?? string.Empty,
                    NormalizeDegrees(commanded.RADegrees),
                    commanded.Dec,
                    NormalizeDegrees(settledAfter.RADegrees),
                    settledAfter.Dec,
            ...
            catch (Exception ex)
            {
                _ = await PublishRunJsonEvidenceAsync(
                    "g3-post-wcs-motion-prediction-unavailable",
                    "The WCS move completed, but no unsolved-target-field prediction was retained",
                    new { reason = ex.Message, currentField.FramePath },
                    currentField.FramePath,
                    cancellationToken).ConfigureAwait(false);
            }
                ...
                currentResidual + currentField.TargetIdentification.CatalogPositionSpreadPixels <= placementPreset.CoarseHandoffResidualPixels)
            {
                state = ReanchorG3AcquisitionMotionFromReportedPosition(state, telescopeMediator.GetCurrentPosition()) with
                {
                    ...
                    CommandMagnitudeArcseconds = 0,
                    UpdatedUtc = DateTimeOffset.UtcNow,
                    LastReason = $"Fresh G3 validation passed after direct-to-slit WCS attempt {attempts} with {currentResidual:F2}px residual; evidence {attemptEvidence}.",
                };
                await PersistG3AcquisitionMotionAsync(state, CancellationToken.None).ConfigureAwait(false);
                // Preserve the same immutable post-move field and target identity
                // for the PHD2 fine-placement stage. The WCS helper owns this local
                // field variable, while the next canonical stage reads lastG3Field.
                lastG3Field = currentField;
                return G3FieldPassed(
                    currentField,
                    transferEvidencePath,
            ...
            if ((currentField.Gate.Code == "G3_SOLVED_TARGET_OUTSIDE" || currentField.Gate.Disposition == GateDisposition.Passed) &&
                currentField.Solve?.Result.Success == true &&
                currentField.Solve.Result.Coordinates is not null &&
                currentField.Image is not null)
            {
                try
                {
                    var nextDestination = currentField.SlitDetection.Gate.Disposition == GateDisposition.Passed
                        ? currentField.SlitDetection.Geometry.AcquisitionPoint
                        : commissioning.SlitGeometry.AcquisitionPoint;
                    var nextProjectionDestination = G3CatalogTargetPositionPolicy.ProjectionDestination(
                        currentField.TargetIdentification, nextDestination, placementPreset.TargetSearchRadiusPixels);
                    var nextInverse = G3WcsTargetProjector.SolveCenterForTargetAtPixel(
                        targetCoordinates,
                        currentField.Solve.Result,
                        currentField.Image.Properties.Width,
                        currentField.Image.Properties.Height,
                        currentField.Solve.SolverIdentity,
                        nextProjectionDestination);
                    var nextCorrection = G3AcquisitionMotionPlanner.SignedTangentOffsetArcseconds(
                        NormalizeDegrees(currentField.Solve.Result.Coordinates.RADegrees),
                        currentField.Solve.Result.Coordinates.Dec,
                        NormalizeDegrees(nextInverse.DesiredG3Center.RADegrees),
                        nextInverse.DesiredG3Center.Dec);
                    nextCatalogResidualPixels = PixelDistance(
                        currentField.TargetIdentification.Target?.Centroid ?? nextInverse.CurrentTargetPixel, nextDestination);
                    ...
                    priorRequiredMotionArcseconds, nextRequiredMotionArcseconds, state.ArrivalToleranceArcseconds))
            {
                ...
                state = ReanchorG3AcquisitionMotionFromReportedPosition(state, telescopeMediator.GetCurrentPosition()) with
                {
                    Phase = G3AcquisitionMotionPhase.AwaitingFreshSolve,
                    UpdatedUtc = DateTimeOffset.UtcNow,
                    LastReason = $"Fresh G3 WCS reduced the remaining direct-to-slit motion from {priorRequiredMotionArcseconds:F2} to {nextRequiredMotionArcseconds:F2} arcsec; the durable return obligation remains outstanding while another bounded segment is planned.",
                };
                await PersistG3AcquisitionMotionAsync(state, CancellationToken.None).ConfigureAwait(false);
                continue;
            }
        ...
        // motion authority after the attested return.  Never propagate that
        // gate across the return boundary.  Reacquire below at the actual
        // origin, just as we do for recoverable failed solves.
        if (currentField.Gate.Disposition != GateDisposition.Passed &&
            !IsRecoverableG3SearchGate(currentField.Gate))
        {
            return new StageResult(
                currentField.Gate,
                summaryPath,
        ...
        {
            // A returned solved field is not automatically ready for fine
            // placement. A completed charged move bounds re-entry; a zero-
            // action inverse/geometry failure must not recurse indefinitely.
            if (attempts > 0 && NeedsG3CoarseCentering(originField))
                return await RunG3WcsCenteringAsync(context, originField, transferEvidencePath,
                    cancellationToken, allowChargedCurrentPositionHandoff: true).ConfigureAwait(false);
            return G3FieldPassed(
                originField,
        ...
        bool allowChargedCurrentPositionHandoff = false)
    {
        ...
        try
        {
            foreach (var waypoint in waypoints)
            {
                ...
                // The search grid is relative to this run's fresh reported
                // origin, while a continued durable ledger retains the older
                // lineage origin. Project the actual spherical command back
                // into the durable TAN frame before reserving or precharging
                // it; directly reusing the grid offsets adds the harmless
                // handoff readback delta to every nominal step.
                var durableWaypointOffset = G3AcquisitionMotionPlanner.SignedTangentOffsetArcseconds(
                    durableSearch.OriginRaDegrees,
                    durableSearch.OriginDeclinationDegrees,
                    commandedCoordinate.RaDegrees,
                    commandedCoordinate.DecDegrees);
                if (!double.IsFinite(durableWaypointOffset.RaArcseconds) ||
                    !double.IsFinite(durableWaypointOffset.DecArcseconds))
                {
                    stopCode = "G3_SEARCH_DURABLE_OFFSET_INVALID";
                    stopReason = "The bounded-search spherical command cannot be represented in the durable lineage TAN frame.";
                    break;
                }
                var durableReserve = G3AcquisitionMotionPlanner.ValidateOutboundAndReturnReserve(
                    durableSearch,
                    durableWaypointOffset.RaArcseconds,
                    durableWaypointOffset.DecArcseconds,
                    DateTimeOffset.UtcNow);
                ...
                if (lastG3Field.Gate.Disposition == GateDisposition.Passed)
                {
                    ...
                    var successEvidencePath = await PublishG3SearchSummaryAsync(
                        NeedsG3CoarseCentering(lastG3Field) ? "TargetInsideHandedToWcsCentering" : "TargetIdentified",
                        directField,
                        search,
                        attempts,
                        ...
                        cancellationToken).ConfigureAwait(false);
                    if (NeedsG3CoarseCentering(lastG3Field))
                    {
                        Report("邻场搜索已找到目标，但离狭缝仍远；沿同一已计账运动链转入 WCS 居中，尚不启动 PHD2 精调。");
                        return await RunG3WcsCenteringAsync(context, lastG3Field, successEvidencePath,
                            cancellationToken, allowChargedCurrentPositionHandoff: true).ConfigureAwait(false);
                    }
                    return G3FieldPassed(
                        lastG3Field,
                ...
                if (lastG3Field.Gate.Code == "G3_SOLVED_TARGET_OUTSIDE" &&
                    lastG3Field.Solve?.Result.Success == true &&
                    lastG3Field.Solve.Result.Coordinates is not null)
                {
                    // This is the route that completed the sparse M76 field:
                    // the target field itself need not solve.  A formally
                    // plausible PL3 solution in an overlapping neighbour is a
                    // much better motion authority than continuing a blind
                    // spiral.  Preserve the original durable origin and every
                    // already consumed arcsecond/attempt, settle only the
                    // completed search command, then hand the charged lineage
                    // to the normal segmented WCS-centering loop.
                    durableSearch = ReanchorG3AcquisitionMotionFromReportedPosition(
                        durableSearch,
                        telescopeMediator.GetCurrentPosition()) with
                    {
                        ...
                        CommandMagnitudeArcseconds = 0,
                        UpdatedUtc = DateTimeOffset.UtcNow,
                        LastReason = $"Neighbouring-field attempt {waypoint.Attempt} produced a formal PL3 WCS outside the target field; the completed search command is settled and its charged lineage is handed directly to WCS centering.",
                    };
                    await PersistG3AcquisitionMotionAsync(durableSearch, CancellationToken.None).ConfigureAwait(false);
                    cumulativeCorrectionDegrees = Math.Max(
                        cumulativeCorrectionDegrees,
                    ...
                    pendingG3SearchReturn = null;
                    var handoffEvidencePath = await PublishG3SearchSummaryAsync(
                        "NeighbourPl3HandedToWcsCentering",
                        directField,
                        search,
                        attempts,
                        returnedToOrigin: false,
                        finalReason: $"{lastG3Field.Gate.Message} Formal PL3 now owns the direct return toward the catalog target; no additional blind neighbour is attempted.",
                        cancellationToken).ConfigureAwait(false);
                    return await RunG3WcsCenteringAsync(
                        context,
                        lastG3Field,
    ...
    private async Task<G3FieldState> CaptureAndAnalyzeG3Async(
        ObservationContext context,
        CancellationToken cancellationToken,
        G3PlateSolveProbeState? trustedSolveProbe = null,
        G3PlateSolveProbeState? solveLadderProbe = null,
        G3WcsMotionPrediction? motionPrediction = null,
        bool slitGeometryOnly = false)
    {
        if (commissioning is null) throw new InvalidOperationException("Commissioning preset is not loaded.");
        ...
        if (slitGeometryOnly && TryGetCurrentG3SlitGeometryRunCache(context, out var preparedCache))
        {
            return PreparedG3SlitGeometryField(preparedCache!);
        }

        // Slit geometry is detector-fixed. Mount motion changes the sky field,
        // not the slit locus. Once this run has measured and optically identified
        // the selected slit, reuse that geometry with each fresh, mount-bound PL3
        // frame until a mechanical/configuration/focus identity changes. This
        // keeps the sky evidence fresh without repeating two 9-frame HDR LED
        // sequences after every WCS correction or neighbouring-field move.
        var cachedGeometryField = await TryAnalyzeFreshG3ProbeWithRunSlitGeometryAsync(
            context,
            trustedSolveProbe ?? solveLadderProbe,
            focusOwnerBefore,
            motionPrediction,
            cancellationToken).ConfigureAwait(false);
        if (cachedGeometryField is not null)
        {
            return cachedGeometryField;
        }
            ...
            .Select(fingerprint =>
            {
                ...
                var analysis = SlitDarkApertureHdrAnalyzer.Analyze(
                    ...
                    new SlitDarkApertureHdrOptions(
                        ...
                        ExpectedReflectiveEdgeToApertureCenterPixels:
                            fingerprint.ReflectiveEdgeToApertureCenterPixels,
                        AllowCommissionedMidpointTransfer:
                            fingerprint.Resolution == SlitDarkApertureResolution.SharedPsfModel,
                        CommissionedApertureWidthPixels: fingerprint.MeasuredWidthPixels,
                        CommissionedWidthUncertaintyPixels: fingerprint.WidthUncertaintyPixels));
        ...
        // Keep the immutable OFF frame as WCS/focus authority, but show the
        // middle accepted LED-ON frame in the operator preview. An expected
        // 10/20 ms OFF exposure is nearly black and previously made a valid
        // slit sequence look like a failed camera readout.
        var slitPreview = shortLoaded.Single(frame =>
            frame.Captured.Phase == G3SlitIlluminationPhase.On &&
            frame.Captured.PhaseIndex == 2);
        ...
        GateResult? trustedPl3CarryGate = null;
        if (trustedSolveProbe is not null)
        {
            trustedPl3CarryGate = await ValidateTrustedG3SolveCarryForwardAsync(
                context,
                trustedSolveProbe,
                reference,
                cancellationToken).ConfigureAwait(false);
            _ = await PublishRunJsonEvidenceAsync(
                "g3-trusted-pl3-carry-forward",
                "Formal target-inside PL3 WCS carried into the no-motion LED slit sequence",
                new
                {
                    disposition = trustedPl3CarryGate.Disposition.ToString(),
                    trustedPl3CarryGate.Code,
                    trustedPl3CarryGate.Message,
                    trustedPl3CarryGate.Metrics,
                    solveFrame = trustedSolveProbe.FramePath,
                    solveEvidence = trustedSolveProbe.Solve?.EvidencePath,
                    solveCenterRaDegrees = trustedSolveProbe.Solve?.Result.Coordinates?.RADegrees,
                    solveCenterDecDegrees = trustedSolveProbe.Solve?.Result.Coordinates?.Dec,
                    solvePixelScaleArcseconds = trustedSolveProbe.Solve?.Result.Pixscale,
                    solvePositionAngleDegrees = trustedSolveProbe.Solve?.Result.PositionAngle,
                    referenceFrame = reference.Captured.Capture.Path,
                    authority = "formal PL3 WCS plus immutable same-detector mount-bound FITS; no source-count veto and no learned interpretation",
                },
                trustedSolveProbe.FramePath,
                cancellationToken).ConfigureAwait(false);
            if (trustedPl3CarryGate.Disposition != GateDisposition.Passed)
            {
                PublishG3Preview(slitPreview.Image, trustedPl3CarryGate.Message, pairAnalysis.Geometry);
                return new G3FieldState(
                    trustedPl3CarryGate,
                    reference.Captured.Capture.Path,
                    image,
                    trustedSolveProbe.Solve,
                    offComposite,
                    trustedSolveProbe.ContentAssessment?.StellarMeasurement.Stars ?? Array.Empty<StarCandidate>(),
                    slitDetection,
        ...
        {
            PublishG3Preview(slitPreview.Image, slitIdentity.Gate.Message, pairAnalysis.Geometry);
            return new G3FieldState(
                slitIdentity.Gate,
                reference.Captured.Capture.Path,
                image,
        ...
        if (pairAnalysis.Gate.Disposition == GateDisposition.Passed &&
            Math.Abs(slitDetection.PerpendicularOffsetPixels) <= maximumCommissionedSlitOffset &&
            Math.Abs(slitDetection.AngleOffsetDegrees) <= 3)
        {
            g3SlitGeometryRunCache = new G3SlitGeometryRunCache(
                context.Plan.ObservationRunId,
                commissioning.Sha256,
                nightSetup.Sha256,
                nightSetup.Value.SlitPosition,
            ...
            await WriteAuditBestEffortAsync("g3-run-slit-geometry-cached", new
            {
                ...
                nightSetup.Value.SlitPosition,
                nightSetup.Value.SlitWidthMicrometers,
                focusPositionSteps = focusOwnerAfter.PositionSteps,
                imageWidth = image.Properties.Width,
                imageHeight = image.Properties.Height,
                slitAcquisitionPoint = slitDetection.Geometry.AcquisitionPoint,
                slitIdentityEvidencePath,
                policy = "run-scoped detector-fixed geometry; mount motion and C11 focus motion do not invalidate; slit/camera/config/run changes do",
            }).ConfigureAwait(false);
        }

        if (slitGeometryOnly)
        {
            if (TryGetCurrentG3SlitGeometryRunCache(context, out var newlyPreparedCache))
            {
                PublishG3Preview(
                    slitPreview.Image,
                    $"G3 已先建立本轮狭缝几何 ({slitDetection.Geometry.AcquisitionPoint.X:F1},{slitDetection.Geometry.AcquisitionPoint.Y:F1})；下一帧开始找星并立即对准。",
                    slitDetection.Geometry);
                return PreparedG3SlitGeometryField(
                    newlyPreparedCache!,
                    reference.Captured.Capture.Path,
                    image,
                    offComposite,
            ...
            var preparationGate = pairAnalysis.Gate.Disposition == GateDisposition.Passed
                ? GateResult.Unknown(
                    "G3_SLIT_GEOMETRY_PREPARATION_REJECTED",
                    $"The fresh HDR slit geometry was measured but lies outside its commissioned detector-fixed envelope: perpendicular offset {slitDetection.PerpendicularOffsetPixels:F2}px (limit {maximumCommissionedSlitOffset:F2}px), angle offset {slitDetection.AngleOffsetDegrees:F2}deg (limit 3.00deg). Target acquisition was not started.")
                : pairAnalysis.Gate;
            return new G3FieldState(
                preparationGate,
                reference.Captured.Capture.Path,
        ...
        {
            // A formal target-inside PL3 solution proves that this stable,
            // Night-Setup-bound C11 field is optically usable.  The short LED
            // frames remain slit-geometry evidence and their sparse star count
            // is telemetry, not a second veto of the already accepted WCS.
            focusGate = GateResult.Pass(
                "G3_MAIN_FOCUS_LOCKED_TRUSTED_PL3",
                $"The C11 focus stayed locked at {focusOwnerAfter.PositionSteps} steps and the same-detector target-inside PL3 solution remained mount-bound through the LED sequence. The {focusMeasurement.DetectedStarCount} short-frame detections are telemetry; the PL3 result is not rejected by a later source-count threshold.",
                C11MainFocusPolicy.MeasurementMetrics(focusMeasurement));
        }
        ...
        if (focusGate.Disposition != GateDisposition.Passed)
        {
                    ...
                    targetMayBeInvisible))
            {
                ...
                var sparseGate = GateResult.Unknown(
                    ...
                    $"That heuristic is diagnostic only: {recoveryAuthority}, so a configured bounded neighbouring-field search may continue. This gate does not itself authorize slit placement or guiding.",
                    C11MainFocusPolicy.MeasurementMetrics(focusMeasurement));
                PublishG3Preview(slitPreview.Image, sparseGate.Message, pairAnalysis.Geometry);
                return new G3FieldState(
                    sparseGate,
                    reference.Captured.Capture.Path,
                    image,
        ...
        var solve = trustedPl3CarryGate?.Disposition == GateDisposition.Passed && trustedSolveProbe?.Solve is { } carriedSolve
            ? carriedSolve
        ...
        if (!solve.Result.Success)
        {
            ...
            if (motionPrediction is not null)
            {
                var placementPreset = commissioning.Value.Phd2SlitPlacement!;
                var predictedTarget = motionPrediction.PredictedTargetPoint;
                var prediction = G3PostWcsPredictionPolicy.Evaluate(
                    predictedTarget,
                    motionPrediction.MaximumUncertaintyPixels,
                    slitDetection.Geometry.AcquisitionPoint,
                    image.Properties.Width,
                    image.Properties.Height,
                    placementPreset.MaximumAcquisitionResidualPixels);
                var predictedResidual = prediction.PredictedTargetToSlitResidualPixels;
                var predictionInside = prediction.PredictionInsideFrame;
                var predictionAuthorized = prediction.Authorized;
                var motionPath = await PublishRunJsonEvidenceAsync(
                    "g3-post-wcs-motion-prediction",
                    predictionAuthorized
                        ? "Fresh target-field slit geometry accepted with the preceding PL3/mount motion prediction"
                        : "Preceding PL3/mount motion prediction did not authorize the unsolved target field",
                    new
                    {
                        predictionAuthorized,
                        prediction.Code,
                        prediction.Message,
                        predictedTarget,
                        measuredRuntimeSlit = slitDetection.Geometry.AcquisitionPoint,
                        predictedTargetToRuntimeSlitPixels = predictedResidual,
                        motionPrediction.MaximumUncertaintyPixels,
                        maximumAcquisitionResidualPixels = placementPreset.MaximumAcquisitionResidualPixels,
                        motionPrediction.SourceFramePath,
                        motionPrediction.SourceSolveEvidencePath,
                        motionPrediction.SourceMountBindingSha256,
                        motionPrediction.CommandedRaDegrees,
                        motionPrediction.CommandedDecDegrees,
                        motionPrediction.SettledRaDegrees,
                        motionPrediction.SettledDecDegrees,
                        motionPrediction.CommandResidualArcseconds,
                        motionPrediction.InverseResidualPixels,
                        targetFieldFrame = reference.Captured.Capture.Path,
                        targetFieldMountBindingSha256 = reference.Captured.MountBinding?.BindingSha256,
                        freshTargetFieldSolveSucceeded = false,
                        authority = "formal plausible PL3 WCS + durably charged N.I.N.A. motion + settled mount readback + fresh LED dark-aperture geometry",
                    },
                    reference.Captured.Capture.Path,
                    cancellationToken).ConfigureAwait(false);
                if (predictionAuthorized)
                {
                    var identification = TargetIdentification.FromCatalogWcs(
                        ...
                        image.Properties.Width,
                        image.Properties.Height,
                        $"The target field need not solve after a direct return from a formally solved overlapping field. The fresh mount-bound frame retains the preceding PL3/mount prediction with {motionPrediction.MaximumUncertaintyPixels:F2}px bounded uncertainty and measures the runtime slit independently.");
                    var motionCaption = $"G3 邻场 PL3 直接回目标：预测目标 ({predictedTarget.X:F1},{predictedTarget.Y:F1})，新测狭缝 ({slitDetection.Geometry.AcquisitionPoint.X:F1},{slitDetection.Geometry.AcquisitionPoint.Y:F1})，残差 {predictedResidual:F1}px；目标场无需再次解算。";
                    PublishG3Preview(slitPreview.Image, motionCaption, slitDetection.Geometry, predictedTarget);
                    await PublishG3AnalysisEvidenceAsync(
                        context,
                        sequence,
                        image,
                        solve,
                        candidates: focusMeasurement.Stars,
                        pairAnalysis,
                        slitSeed,
                        identification,
                        predictedTarget,
                    ...
                    return new G3FieldState(
                        GateResult.Pass(
                            ...
                            {
                                ["predictedTargetToRuntimeSlitPixels"] = predictedResidual,
                                ["predictionUncertaintyPixels"] = motionPrediction.MaximumUncertaintyPixels,
                                ["freshTargetFieldSolveRequired"] = 0,
                            }),
                        reference.Captured.Capture.Path,
                        image,
                        solve,
                ...
                PublishG3Preview(
                    slitPreview.Image,
                    $"邻场 PL3 回目标预测未通过：预测点到新测狭缝 {predictedResidual:F1}px，预测不确定度 {motionPrediction.MaximumUncertaintyPixels:F1}px，门限 {placementPreset.MaximumAcquisitionResidualPixels:F1}px。",
                    slitDetection.Geometry,
                    predictionInside ? predictedTarget : null);
                return new G3FieldState(
                    GateResult.Unknown(
                        "G3_POST_WCS_PREDICTION_OUTSIDE_ACQUISITION_WINDOW",
                        $"The target-field solve failed and the preceding PL3/mount prediction is {predictedResidual:F2}px from the fresh runtime slit with {motionPrediction.MaximumUncertaintyPixels:F2}px uncertainty, outside the commissioned {placementPreset.MaximumAcquisitionResidualPixels:F2}px acquisition window."),
                    reference.Captured.Capture.Path,
                    image,
                    solve,
                    offComposite,
            ...
            PublishG3Preview(slitPreview.Image, "G3 九帧照明序列已保留，但 OFF 星场解算失败；禁止入缝运动。", pairAnalysis.Geometry);
            await PublishG3AnalysisEvidenceAsync(
                context,
                sequence,
                image,
        ...
        var properties = image.Properties;
        var candidates = trustedPl3CarryGate?.Disposition == GateDisposition.Passed &&
                         trustedSolveProbe?.ContentAssessment?.StellarMeasurement is { } carriedMeasurement
            ? carriedMeasurement.Stars
            : focusMeasurement.Stars;
        if (pairAnalysis.Gate.Disposition != GateDisposition.Passed)
        {
            PublishG3Preview(slitPreview.Image, pairAnalysis.Gate.Message, pairAnalysis.Geometry);
            await PublishG3AnalysisEvidenceAsync(
                context,
                sequence,
                image,
            ...
            Math.Abs(slitDetection.AngleOffsetDegrees) > 3)
        {
            ...
            PublishG3Preview(slitPreview.Image, invalid.Message, slitDetection.Geometry);
            await PublishG3AnalysisEvidenceAsync(
                context,
                sequence,
                image,
    ...
    private bool TryGetCurrentG3SlitGeometryRunCache(
        ObservationContext context,
        out G3SlitGeometryRunCache? cache)
    {
        cache = g3SlitGeometryRunCache;
        if (cache is null || commissioning is null || nightSetup is null)
        {
            return false;
        }
        ...
        return string.Equals(cache.ObservationRunId, context.Plan.ObservationRunId, StringComparison.Ordinal) &&
               ...
               Math.Abs(cache.SlitWidthMicrometers - nightSetup.Value.SlitWidthMicrometers) <= 0.01 &&
               string.Equals(cache.G3CameraStableId, configuration.Phd2.CameraStableId, StringComparison.Ordinal) &&
               cache.Binning == configuration.G3.Binning;
    }

    private static G3FieldState PreparedG3SlitGeometryField(
        G3SlitGeometryRunCache cache,
        string? framePath = null,
        IImageData? image = null,
        MonochromeFrame? frame = null,
        G3FieldMountBinding? mountBinding = null) => new(
            GateResult.Pass(
                "G3_SLIT_GEOMETRY_PREPARED_BEFORE_TARGET",
                $"Run-scoped detector-fixed slit geometry is ready from {cache.CapturedUtc:O}; target acquisition may now begin without a post-identification HDR delay.",
                new Dictionary<string, double>
                {
                    ["slitAcquisitionX"] = cache.SlitDetection.Geometry.AcquisitionPoint.X,
                    ["slitAcquisitionY"] = cache.SlitDetection.Geometry.AcquisitionPoint.Y,
                    ["slitContrastSigma"] = cache.SlitDetection.ContrastSigma,
                }),
            framePath ?? cache.SourceFramePath,
            image,
            null,
            frame,
            Array.Empty<StarCandidate>(),
            ...
            new TargetIdentification(
                GateResult.Unknown("TARGET_NOT_ACQUIRED_YET", "Target acquisition starts after slit geometry preparation."),
                null,
                new PixelPoint(double.NaN, double.NaN),
                double.NaN,
                0),
            MountBinding: mountBinding,
            SlitIdentity: cache.SlitIdentity,
            SlitIdentityEvidencePath: cache.SlitIdentityEvidencePath);

    private async Task<G3FieldState?> TryAnalyzeFreshG3ProbeWithRunSlitGeometryAsync(
        ObservationContext context,
        G3PlateSolveProbeState? probe,
        C11MainFocusOwnerSnapshot focusOwnerBefore,
        G3WcsMotionPrediction? motionPrediction,
        CancellationToken cancellationToken)
    {
        var cache = g3SlitGeometryRunCache;
        if (cache is null || commissioning is null || nightSetup is null ||
            probe?.Image is null || probe.MountBinding is null ||
            string.IsNullOrWhiteSpace(probe.FramePath))
        {
            return null;
        }
        ...
        if (!string.Equals(cache.ObservationRunId, context.Plan.ObservationRunId, StringComparison.Ordinal) ||
            ...
            !string.Equals(cache.G3CameraStableId, configuration.Phd2.CameraStableId, StringComparison.Ordinal) ||
            cache.Binning != configuration.G3.Binning ||
            cache.ImageWidthPixels != probe.Image.Properties.Width ||
            cache.ImageHeightPixels != probe.Image.Properties.Height)
        {
            return null;
        }
        ...
        var properties = probe.Image.Properties;
        var raw = probe.Image.Data.FlatArray;
        if (raw.Length != properties.Width * properties.Height)
        {
            return null;
        }
        var frame = G3FrameInputPolicy.Create(properties.Width, properties.Height, raw, configuration.G3);
        var focusMeasurement = probe.ContentAssessment?.StellarMeasurement ?? G3StellarFocusAnalyzer.Analyze(frame);
        var candidates = focusMeasurement.Stars;
        if (motionPrediction is null &&
            probe.Gate.Disposition != GateDisposition.Passed &&
            (configuration.G3.EffectiveBrightTarget.Enabled ||
             configuration.G3.GhostAssistanceMode != GhostAssistanceMode.Skip))
        {
            // The cache optimization must not make explicitly authorized
            // exceptional target analysis unreachable. Fall through to the
            // complete immutable sequence, which owns the extra short exposure,
            // QHY/catalog/focus authority gates and ghost-template evidence.
            // This is deliberately separate from the post-WCS prediction above:
            // a valid motion prediction needs neither another HDR sequence nor
            // morphology to authorize the PHD2 hand-off.
            return null;
        }
        if (probe.Gate.Disposition != GateDisposition.Passed ||
            probe.Solve?.Result.Success != true || probe.Solve.Result.Coordinates is null)
        {
            if (motionPrediction is not null)
            {
                var placementPreset = commissioning.Value.Phd2SlitPlacement!;
                ...
                var prediction = G3PostWcsPredictionPolicy.Evaluate(
                    predictedTarget,
                    motionPrediction.MaximumUncertaintyPixels,
                    cache.SlitDetection.Geometry.AcquisitionPoint,
                    properties.Width,
                    properties.Height,
                    placementPreset.MaximumAcquisitionResidualPixels);
                var predictedResidual = prediction.PredictedTargetToSlitResidualPixels;
                ...
                var predictionAuthorized = prediction.Authorized;
                var motionPath = await PublishRunJsonEvidenceAsync(
                    "g3-post-wcs-motion-prediction-cached-slit",
                    predictionAuthorized
                        ? "Fresh unsolved target field accepted with preceding PL3/mount prediction and run-cached slit geometry"
                        : "Preceding PL3/mount prediction did not authorize the fresh unsolved target field with run-cached slit geometry",
                    new
                    {
                        predictionAuthorized,
                        prediction.Code,
                        prediction.Message,
                        predictedTarget,
                        cachedRuntimeSlit = cache.SlitDetection.Geometry.AcquisitionPoint,
                        predictedTargetToRuntimeSlitPixels = predictedResidual,
                        motionPrediction.MaximumUncertaintyPixels,
                        maximumAcquisitionResidualPixels = placementPreset.MaximumAcquisitionResidualPixels,
                        motionPrediction.SourceFramePath,
                        ...
                        freshTargetFieldSolveSucceeded = false,
                        repeatedHdrRequired = false,
                        authority = "formal plausible neighbour PL3 WCS + durably charged N.I.N.A. motion + settled mount readback + fresh immutable target frame + unchanged run-cached detector-fixed slit geometry",
                    },
                    probe.FramePath,
                    cancellationToken).ConfigureAwait(false);
                if (predictionAuthorized)
                    ...
                    return new G3FieldState(
                        GateResult.Pass(
                            ...
                            new Dictionary<string, double>
                            {
                                ...
                                ["predictionUncertaintyPixels"] = motionPrediction.MaximumUncertaintyPixels,
                                ["freshTargetFieldSolveRequired"] = 0,
                                ["slitHdrRepeatedForMountMotion"] = 0,
                            }),
                        probe.FramePath,
                        probe.Image,
                        probe.Solve,
                        ...
                        identification,
                        focusMeasurement,
                        MountBinding: probe.MountBinding,
                        SlitIdentity: cache.SlitIdentity,
                        SlitIdentityEvidencePath: cache.SlitIdentityEvidencePath);
                }

                var predictionGate = GateResult.Unknown(
                    "G3_POST_WCS_PREDICTION_OUTSIDE_ACQUISITION_WINDOW",
                    $"{prediction.Code}: {prediction.Message} The target-field solve failed and the preceding PL3/mount prediction is {predictedResidual:F2}px from the run-cached slit with {motionPrediction.MaximumUncertaintyPixels:F2}px uncertainty (commissioned window {placementPreset.MaximumAcquisitionResidualPixels:F2}px). The mount may return safely, but this lineage must not restart generic neighbouring-field search.");
                PublishG3Preview(
                    probe.Image,
                    $"邻场 PL3 回目标预测未通过：预测点到缓存狭缝 {predictedResidual:F1}px，预测不确定度 {motionPrediction.MaximumUncertaintyPixels:F1}px，门限 {placementPreset.MaximumAcquisitionResidualPixels:F1}px；安全返回后停止本 lineage，不再循环邻场搜索。",
                    cache.SlitDetection.Geometry,
                    predictionInside ? predictedTarget : null);
                return new G3FieldState(
                    predictionGate,
            ...
            var unsolvedCaption = $"G3 fresh 星场沿用本轮缓存狭缝 ({cache.SlitDetection.Geometry.AcquisitionPoint.X:F1},{cache.SlitDetection.Geometry.AcquisitionPoint.Y:F1})；本次 PL3 未解算，不重复 HDR，继续既有亮目标/邻场退化判定。";
            PublishG3Preview(probe.Image, unsolvedCaption, cache.SlitDetection.Geometry);
            var unsolvedReuseEvidencePath = await PublishRunJsonEvidenceAsync(
                "g3-run-slit-geometry-reused-unsolved",
                "Unsolved fresh mount-bound G3 field retained with run-scoped detector-fixed slit geometry",
                new
                {
                    context.Plan.ObservationRunId,
                    currentFrame = probe.FramePath,
                    currentMountBindingSha256 = probe.MountBinding.BindingSha256,
                    currentGateDisposition = probe.Gate.Disposition.ToString(),
                    probe.Gate.Code,
                    probe.Gate.Message,
                    cachedGeometryCapturedUtc = cache.CapturedUtc,
                    cachedGeometrySourceFrame = cache.SourceFramePath,
                    cachedSlitIdentityEvidencePath = cache.SlitIdentityEvidencePath,
                    cachedSlitAcquisitionPoint = cache.SlitDetection.Geometry.AcquisitionPoint,
                    policy = "PL3 failure does not invalidate detector-fixed slit geometry; no repeat HDR before configured recovery",
                    invalidatedByMountMotion = false,
                },
                probe.FramePath,
                cancellationToken).ConfigureAwait(false);
            await WriteAuditBestEffortAsync("g3-run-slit-geometry-reused-unsolved", new
            {
                context.Plan.ObservationRunId,
                currentFrame = probe.FramePath,
                reuseEvidencePath = unsolvedReuseEvidencePath,
                probe.Gate.Code,
            }).ConfigureAwait(false);
            return new G3FieldState(
                probe.Gate,
        ...
        PublishG3Preview(
            ...
            caption,
            cache.SlitDetection.Geometry,
            identified.Target?.Centroid ?? predictedPoint);

        var reuseEvidencePath = await PublishRunJsonEvidenceAsync(
            "g3-run-slit-geometry-reused",
            "Fresh mount-bound PL3 field analyzed with run-scoped detector-fixed slit geometry",
            new
            {
                context.Plan.ObservationRunId,
                currentFrame = probe.FramePath,
                currentMountBindingSha256 = probe.MountBinding.BindingSha256,
                currentSolveEvidencePath = probe.Solve.EvidencePath,
                cachedGeometryCapturedUtc = cache.CapturedUtc,
                cachedGeometrySourceFrame = cache.SourceFramePath,
                cachedSlitIdentityEvidencePath = cache.SlitIdentityEvidencePath,
                cachedSlitAcquisitionPoint = cache.SlitDetection.Geometry.AcquisitionPoint,
                predictedTarget = predictedPoint,
                adoptedTarget = identified.Target?.Centroid,
                identified.CatalogPositionRefinedFromSameFrame,
                identified.PredictionResidualPixels,
                targetToSlitResidualPixels = double.IsFinite(residualPixels) ? residualPixels : (double?)null,
                targetDisposition = identified.Gate.Disposition.ToString(),
                identified.Gate.Code,
                policy = "fresh sky/WCS every mount position; HDR slit geometry once per unchanged run/mechanical/focus/camera identity",
                invalidatedByMountMotion = false,
            },
            probe.FramePath,
            cancellationToken).ConfigureAwait(false);
        await WriteAuditBestEffortAsync("g3-run-slit-geometry-reused", new
        {
            context.Plan.ObservationRunId,
            currentFrame = probe.FramePath,
            reuseEvidencePath,
        ...
        var gate = identified.Gate.Disposition == GateDisposition.Passed
            ? GateResult.Pass(
                "G3_FIELD_ANALYZED_RUN_SLIT_GEOMETRY",
                caption,
                new Dictionary<string, double>
                {
                    ["targetPredictionResidualPixels"] = residualPixels,
                    ["slitContrastSigma"] = cache.SlitDetection.ContrastSigma,
                    ["slitConfidence"] = cache.PairAnalysis.Confidence,
                    ["detectedStars"] = candidates.Count,
                    ["slitHdrRepeatedForMountMotion"] = 0,
                })
            : identified.Gate;
        ...
        CancellationToken cancellationToken)
    {
        ...
        var captured = await CaptureG3FullFrameForAcquisitionAsync(
            new Phd2SingleFrameRequest(
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await G3AtomicLedOnBlock.ExecuteAsync(
                ...
                async token =>
                {
                    ...
                    await PrepareG3IlluminationSingleFramePhaseAsync(
                        context,
            ...
            try
            {
                ...
                PublishEvidencePathOnce(
                    ...
                    new Dictionary<string, string>
                    {
                        ...
                        ["gainPercent"] = G3SlitIlluminationPolicy.CaptureGainPercent.ToString(CultureInfo.InvariantCulture),
                        ["binning"] = configuration.G3.Binning.ToString(CultureInfo.InvariantCulture),
        ...
        await PublishG3SlitSequenceEvidenceAsync(
            sequence,
            exposureMilliseconds,
            exposureRole,
            CancellationToken.None).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken)
    {
        ...
        // OFF phases may honour a requested pause after every native single
        // frame has completed. ON does the equivalent only after verified OFF.
        await CheckpointAndRejectStaleStageStackAsync(context, cancellationToken).ConfigureAwait(false);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        // CommandActiveSlitIlluminationAsync has already verified the UVEX
        // output state and photodiode. This guard prevents a native single
        // frame from straddling the LED transition.
        var ledSettleMilliseconds = Math.Clamp(exposureMilliseconds * 3, 250, 1_000);
        await Task.Delay(TimeSpan.FromMilliseconds(ledSettleMilliseconds), cancellationToken).ConfigureAwait(false);

        Report($"PHD2/G3 狭缝照明 HDR {exposureMilliseconds} ms · gain {G3SlitIlluminationPolicy.CaptureGainPercent}% · {PhaseDisplayName(phase)} · 原生单帧采集");
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var captured = await CaptureG3NativeSingleFrameForAcquisitionAsync(
            new Phd2SingleFrameRequest(
                exposureMilliseconds,
                configuration.G3.Binning,
                G3SlitIlluminationPolicy.CaptureGainPercent,
                path),
            cancellationToken).ConfigureAwait(false);
        ...
        if (!captured.GainAndBinningApplied)
        {
            throw new InvalidOperationException(
                $"PHD2 did not attest the requested gain/binning for {role}; profile-gain fallback is prohibited for slit illumination.");
        }
        if (phase == G3SlitIlluminationPhase.On)
        {
            ...
            PublishG3Preview(
                previewImage,
                $"G3 狭缝照明 HDR {exposureMilliseconds} ms · gain {G3SlitIlluminationPolicy.CaptureGainPercent}% · LED ON {index}/{G3SlitIlluminationPolicy.FramesPerPhase} · 原生单帧已保存。");
        }
        else
        {
            Report($"G3 狭缝照明 HDR {exposureMilliseconds} ms · gain {G3SlitIlluminationPolicy.CaptureGainPercent}% · {PhaseDisplayName(phase)} {index}/{G3SlitIlluminationPolicy.FramesPerPhase} · 暗证据帧已保存（不替换实时狭缝图）。");
        }
        ...
        IImageData image)
    {
        ...
        if (!captured.Capture.GainAndBinningApplied)
        {
            return GateResult.Unknown(
                "G3_GAIN_BINNING_NOT_APPLIED",
                $"PHD2 did not attest native single-frame gain/binning for {captured.Role}.");
        }
        ...
        CancellationToken cancellationToken)
    {
        await PublishRunJsonEvidenceAsync(
            ...
            new
            {
                ...
                capturePolicy = new
                {
                    exposureMilliseconds = requestedExposureMilliseconds,
                    gainPercent = G3SlitIlluminationPolicy.CaptureGainPercent,
                    exposureRole,
                    framesPerPhase = G3SlitIlluminationPolicy.FramesPerPhase,
                    phases = new[] { "off-before", "on", "off-after" },
                    transitionGuard = "verified LED readback followed by a bounded settle before native single-frame capture",
                    captureMode = "PHD2 capture_single_frame applies exposure, binning and minimum gain atomically; no profile or registry mutation",
                    motionDuringSequence = "none commanded by UVEX-ADV",
                    postLedReadbackSettleMilliseconds = Math.Clamp(requestedExposureMilliseconds * 3, 250, 1_000),
    ...
    private async Task<StageResult> StartOptionalQhyPhotometryAsync(
        ObservationContext context,
        CancellationToken cancellationToken)
    {
        ...
        {
            return Warning(
                "QHY_PHOTOMETRY_SKIPPED",
                $"Optional synchronized QHY photometry is unavailable ({qhyUnavailableReason ?? "reason unavailable"}); settled ATR spectroscopy will continue.");
        }

        try
        {
            var result = await StartQhyPhotometryAsync(context, cancellationToken).ConfigureAwait(false);
            if (result.CanAdvance) return result;
            if (!result.Gate.Code.StartsWith("QHY_", StringComparison.Ordinal)) return result;
            DisableQhyForRun($"{result.Gate.Code}: {result.Gate.Message}");
            return Warning(
                "QHY_PHOTOMETRY_DEGRADED",
                $"Synchronized QHY photometry could not be started ({qhyUnavailableReason}). ATR spectroscopy remains active and will record the missing photometry coverage.",
                result.Gate.Metrics,
                result.Metadata);
        }
        catch (OperationCanceledException) { throw; }
        catch (PhysicalActionGateException) { throw; }
        catch (Phd2GuideOutputException) { throw; }
        catch (Exception ex)
        {
            DisableQhyForRun($"QHY photometry raised {ex.GetType().Name}: {ex.Message}");
            return Warning(
                "QHY_PHOTOMETRY_EXCEPTION_DEGRADED",
                $"{qhyUnavailableReason} ATR spectroscopy remains active without synchronized QHY coverage.");
        }
    }
    ...
    private async Task<StageResult> StartQhyPhotometryAsync(ObservationContext context, CancellationToken cancellationToken)
    {
        ...
        var request = new PhotometryJobRequest(
            ...
            FilterName: configuration.Qhy.FilterName,
            TargetTemperatureC: configuration.Qhy.TargetTemperatureC,
            // Cloud, FWHM and transparency are frame-quality metadata, not a
            // reason to stop the simultaneous spectrum. Preserve and flag the
            // QHY frame; only an actual camera/service failure disables this
            // optional channel.
    ...
    private async Task<StageResult> SelectAtrExposureAsync(ObservationContext context, CancellationToken cancellationToken)
    {
        ...
        while (visitBudget.AttemptCount < visitBudget.MaximumAttempts)
        {
            ...
            var decision = ExposureTierSelector.Select(probe.Metrics, exposureOptions);
            ...
            {
                // The two successful unattended on-sky loops did not treat an
                // underexposed first probe as proof that a faint target was
                // absent.  If the current immutable frame is demonstrably not
                // clipped, advance exactly one configured tier and measure
                // again.  Invalid telemetry, unstable guiding and exhausted
                // ladders remain hard stops; this is not permission to infer a
                // science exposure from a low-signal frame.
                if (decision.Code is "TARGET_SKY_CONTRAST_LOW" or "NO_POSITIVE_SPECTRAL_SIGNAL" &&
                    IsSafeSignalLimitedAtrProbe(probe.Metrics, exposureOptions))
                {
                    var nextLonger = availableTiers
                        .Where(value => value > probeExposure)
                        .OrderBy(value => value)
                        .Cast<double?>()
                        .FirstOrDefault();
                    if (nextLonger is { } nextExposure)
                    {
                        await WriteAuditBestEffortAsync("atr-signal-limited-advance", new
                        {
                            context.Plan.ObservationRunId,
                            attemptedAtrProbeFrames,
                            fromExposureSeconds = probeExposure,
                            toExposureSeconds = nextExposure,
                            decision.Code,
                            decision.Reason,
                            currentFrameClipped = false,
                            policy = "advance-one-configured-tier-and-remeasure",
                        }).ConfigureAwait(false);
                        Report($"ATR {probeExposure:G4}s 探针信号不足但未削顶；沿安全阶梯重拍 {nextExposure:G4}s，不要求人工恢复");
                        probeExposure = nextExposure;
                        continue;
                    }
                    ...
                    await WriteAuditBestEffortAsync("atr-signal-limited-longest-safe-tier", new
                    {
                        context.Plan.ObservationRunId,
                        attemptedAtrProbeFrames,
                        selectedExposureSeconds = probeExposure,
                        decision.Code,
                        decision.Reason,
                        currentFrameClipped = false,
                        policy = "continue-and-stack-at-longest-safe-tier",
                    }).ConfigureAwait(false);
                    return Warning(
                        "ATR_SIGNAL_LIMITED_LONGEST_SAFE_TIER",
                        $"The target remains signal-limited at the longest configured safe, unclipped tier ({probeExposure:G4}s). This is expected for faint targets: continue science acquisition and improve SNR by stacking instead of blocking the run.",
                        decision.Metrics);
                }
    ...
    private static bool IsSafeSignalLimitedAtrProbe(
        SpectralProbeMetrics metrics,
        ExposureTierOptions options)
    {
        if (!double.IsFinite(metrics.BiasLevelAdu) ||
            !double.IsFinite(metrics.HighPercentileAdu) ||
            !double.IsFinite(metrics.FullScaleAdu) ||
            metrics.FullScaleAdu <= metrics.BiasLevelAdu ||
            !double.IsFinite(metrics.SaturatedFraction) ||
            !double.IsFinite(metrics.TraceSaturatedFraction) ||
            !double.IsFinite(metrics.ClippedDispersionColumnFraction))
        {
            return false;
        }

        var usableRange = metrics.FullScaleAdu - metrics.BiasLevelAdu;
        var observedFraction = Math.Max(0, metrics.HighPercentileAdu - metrics.BiasLevelAdu) / usableRange;
        return metrics.SaturatedFraction <= options.MaximumSaturatedFraction &&
            metrics.TraceSaturatedFraction <= options.MaximumTraceSaturatedFraction &&
            metrics.ClippedDispersionColumnFraction <= options.MaximumClippedDispersionColumnFraction &&
            metrics.LongestClippedDispersionColumnRun <= options.MaximumConsecutiveClippedDispersionColumns &&
            observedFraction < options.MaximumFullScaleFraction;
    }
    ...
    private async Task<StageResult> RunScienceBlockAsync(ObservationContext context, CancellationToken cancellationToken)
    {
        ...
        while (savedAtrFrames < configuration.Atr.ScienceFrameCount)
        {
            ...
            if (qhyGate.Severity == GateSeverity.Warning)
            {
                await WriteAuditBestEffortAsync("qhy-photometry-optional-warning", new
                {
                    context.Plan.ObservationRunId,
                    qhyGate.Code,
                    qhyGate.Message,
                    atrAttempt = attemptedAtrFrames + 1,
                    spectroscopyContinues = true,
                }).ConfigureAwait(false);
            }
            ...
            if (quality.Disposition != GateDisposition.Passed)
            {
                ...
                // loop and prevents an endless probe/capture cycle.
                continue;
            }
            savedAtrFrames++;
    ...
    private async Task<StageResult> FinalizeObservationAsync(ObservationContext context, CancellationToken cancellationToken)
    {
        ...
        {
            var (snapshot, readbackIssue) = await ReadQhyJobForFinalizationAsync(
                jobId,
                cancellationToken).ConfigureAwait(false);
            if (readbackIssue is not null)
            {
                // QHY is the optional parallel channel. A transient service
                // readback failure must not bypass the required PHD2, cover and
                // roof cleanup that follows. Keep the lifecycle handle so a
                // later cleanup/dispose pass can still reconcile it.
                cleanupWarnings.Add(readbackIssue);
            }
            ...
            if (snapshot is not null && snapshot.State is not (QhyJobState.Completed or QhyJobState.Cancelled or QhyJobState.Faulted or QhyJobState.TakenOver))
            {
                try
                {
                    // Keep the optional-owner checkpoint inside the optional
                    // reconciliation boundary. An ordinary checkpoint/service
                    // fault becomes a QHY warning and cannot bypass required
        ...
        var slitOff = await EnsureSlitIlluminationOffAsync(
            ...
            releaseLeaseOnSuccess: true,
            cancellationToken).ConfigureAwait(false);
        if (slitOff.Issue is not null) cleanupErrors.Add(slitOff.Issue);
        ...
        if (configuration.Environment.CloseDomeOrRoofOnFinalize)
        {
            ...
            var roofIssue = await ParkMountAndCloseDomeOrRoofAsync(
                "Normal observation finalization.",
                cancellationToken).ConfigureAwait(false);
            if (roofIssue is not null) cleanupErrors.Add(roofIssue);
        }
        ...
        {
            return Attention(ObservationStage.FinalizeObservation, "FINALIZE_INCOMPLETE", string.Join(" ", cleanupErrors));
        }
        var finalMetrics = new Dictionary<string, double>
        {
            ["atrAcceptedFrames"] = savedAtrFrames,
            ["atrRetainedFrames"] = retainedAtrScienceFrames,
            ["atrAttemptedFrames"] = attemptedAtrFrames,
            ["atrWarningFrames"] = atrWarningFrames,
            ["cumulativeCorrectionArcseconds"] = cumulativeCorrectionDegrees * 3600,
            ["correctionAttempts"] = correctionAttempts,
        };
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (var attempt = 1; attempt <= 2; attempt++)
        {
            try
            {
                ...
                if (snapshot is not null)
                {
                    if (attempt > 1)
                    {
                        await WriteAuditBestEffortAsync("qhy-finalize-readback-recovered", new
                        {
                            jobId,
                            attempt,
                            firstFailure = firstFailure?.Message,
                            readOnlyRetry = true,
                            qhyCameraReconnectAttempted = false,
                        }).ConfigureAwait(false);
                    }
                    return (snapshot, null);
                }
                firstFailure ??= new InvalidOperationException(
                    $"QHY service returned no job snapshot for {jobId:D}.");
            }
            catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
            {
                throw;
        ...
        await WriteAuditBestEffortAsync("qhy-finalize-readback-degraded", new
        {
            jobId,
            attempts = 2,
            failure = firstFailure?.Message,
            optionalChannel = true,
            requiredCleanupContinues = true,
            qhyCameraReconnectAttempted = false,
        }).ConfigureAwait(false);
        return (
            null,
            $"QHY photometry terminal readback remained unavailable after two read-only attempts: {firstFailure?.Message ?? "no snapshot"}. Required PHD2/cover/roof cleanup continued.");
    }
    ...
    private IReadOnlyList<string> ValidateStaticConfiguration(ObservationPlan plan)
    {
        ...
        var environmentSelections = configuration.Environment.Devices.ValidateForMode(
            configuration.Environment.WeakSupervisionEnabled);
        if (environmentSelections.Disposition != GateDisposition.Passed)
        {
            issues.Add(environmentSelections.Message);
        }
        ...
        {
            issues.Add("G3 selected-sky-hint trust radius must be within (0, 30] degrees and is independent of the smaller correction-motion limits. A target/mount residual must never be copied into this value as an optical-axis offset.");
        }
        ...
        if (configuration.Environment.DomeOrRoofTransitionTimeoutSeconds is < 15 or > 900)
        {
            issues.Add("Dome/roll-off-roof transition timeout must be within [15, 900] seconds.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            captureToken, exposureSeconds,
            beforeConnection = guideBeforeExposure.ConnectionEpoch, beforeGuide = guideBeforeExposure.GuideEpoch,
            afterConnection = guideAfterExposure.ConnectionEpoch, afterGuide = guideAfterExposure.GuideEpoch,
            state = guideAfterExposure.AppState.ToString(), guideAfterExposure.LastConfigurationChange,
            guideOutputFailed = guideAfterExposure.GuideOutput?.Failed == true,
        }).ConfigureAwait(false);
        ...
        if (guideBeforeExposure.ConnectionEpoch != guideAfterExposure.ConnectionEpoch ||
            guideBeforeExposure.GuideEpoch != guideAfterExposure.GuideEpoch)
            metrics = metrics with { GuidingStable = false };
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (temperatureGate.Disposition != GateDisposition.Passed)
            {
                ...
                qualityDisposition = temperatureGate.Disposition;
                qualityCode = temperatureGate.Code;
    ...
    private bool IsGuidingStable()
    {
        ...
        return snapshot.GuideOutput?.Failed != true &&
            (snapshot.HasCurrentSuccessfulSettle || acceptedGuidingWindow) &&
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return GateResult.Warn(
                "QHY_PHOTOMETRY_UNAVAILABLE",
                $"No synchronized QHY photometry is available ({qhyUnavailableReason ?? "reason unavailable"}); ATR spectroscopy continues with missing-coverage provenance.");
        }
        if (!qhyLeaseFailures.IsEmpty)
        {
            DisableQhyForRun($"QHY control lease became unhealthy: {string.Join(" ", qhyLeaseFailures.Values)}");
            return GateResult.Warn("QHY_CONTROL_LEASE_UNHEALTHY", qhyUnavailableReason!);
        }
        if (photometryJobId is not { } jobId)
        {
            DisableQhyForRun("No synchronized QHY photometry job is registered.");
            return GateResult.Warn("QHY_PHOTOMETRY_MISSING", qhyUnavailableReason!);
        }
        QhyJobSnapshot? snapshot;
        try
        {
            snapshot = await qhy.GetJobAsync(jobId, cancellationToken).ConfigureAwait(false);
            if (snapshot is not null)
            {
        ...
        {
            DisableQhyForRun($"QHY photometry health/preview became unavailable: {ex.Message}");
            await WriteAuditBestEffortAsync("qhy-photometry-health-degraded", new
            {
                jobId,
                exception = ex.ToString(),
                spectroscopyContinues = true,
                lifecycleHandleRetainedForFinalization = true,
            }).ConfigureAwait(false);
            return GateResult.Warn(
                "QHY_PHOTOMETRY_HEALTH_UNAVAILABLE_DEGRADED",
                $"{qhyUnavailableReason} The optional channel was isolated; ATR spectroscopy continues and the owner job remains registered for checked finalization.");
        }
        if (snapshot is null)
        {
        ...
        if (snapshot.State == QhyJobState.Completed || QhyServiceClient.IsAcquisitionPriorityYield(snapshot))
        {
            ...
            var continuation = await StartOptionalQhyPhotometryAsync(context, cancellationToken).ConfigureAwait(false);
            return continuation.Gate;
        }
        if (snapshot.State is QhyJobState.Running or QhyJobState.Queued)
        {
            return GateResult.Pass("QHY_PHOTOMETRY_HEALTHY", $"QHY photometry is {snapshot.State} with {snapshot.Frames.Count} immutable FITS.");
        }

        var reason = snapshot.State switch
        {
            QhyJobState.PausedNeedsAttention => snapshot.AttentionReason ?? "QHY photometry quality gate requested attention.",
            QhyJobState.Paused or QhyJobState.Pausing => "QHY photometry is paused.",
            _ => snapshot.Error ?? $"QHY photometry is {snapshot.State}.",
        };
        DisableQhyForRun(reason);
        return GateResult.Warn(
            "QHY_PHOTOMETRY_DEGRADED_DURING_SCIENCE",
            $"{reason} This optional channel was isolated; ATR spectroscopy continues.");
    }
    ...
    private async Task<PreStageRecoveryFailure?> RecoverInterruptedStageAsync(
        ObservationStage stage,
        ObservationContext context,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var qhyField = await AcquireOptionalQhyWideFieldAsync(context, cancellationToken).ConfigureAwait(false);
            if (!qhyField.CanAdvance)
                return new PreStageRecoveryFailure(ObservationStage.AcquireQhyWideField, qhyField);
        }
        ...
        if (recovery.ReplaceTargetOnSlit)
        {
            ...
            if (!slit.CanAdvance)
                return new PreStageRecoveryFailure(ObservationStage.PlaceTargetOnSlit, slit);
        }
        if (recovery.RestartGuiding)
        {
            var guiding = await StartGuidingAsync(context, cancellationToken).ConfigureAwait(false);
            if (!guiding.CanAdvance)
                return new PreStageRecoveryFailure(ObservationStage.StartGuiding, guiding);
        }
        if (recovery.RestorePhotometry)
        {
            ...
            if (!photometry.CanAdvance)
                return new PreStageRecoveryFailure(ObservationStage.StartQhyPhotometry, photometry);
        }
    ...
    private sealed record PreStageRecoveryFailure(
        ObservationStage FailedStage,
        StageResult Result);
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return Warning(
                "QHY_PHOTOMETRY_RESUME_SKIPPED",
                $"Optional QHY photometry remains unavailable ({qhyUnavailableReason ?? "reason unavailable"}); resume recovery continues with ATR spectroscopy only.");
        }

        try
        {
            var result = await RestorePhotometryAfterResumeCoreAsync(context, cancellationToken).ConfigureAwait(false);
            if (result.CanAdvance) return result;
            DisableQhyForRun($"{result.Gate.Code}: {result.Gate.Message}");
            return Warning(
                "QHY_PHOTOMETRY_RESUME_DEGRADED",
                $"Optional QHY photometry could not be restored ({qhyUnavailableReason}); fresh G3/slit/guiding recovery remains valid and ATR spectroscopy continues.");
        }
        catch (OperationCanceledException) { throw; }
        catch (PhysicalActionGateException) { throw; }
        catch (Exception ex)
        {
            DisableQhyForRun($"QHY photometry resume raised {ex.GetType().Name}: {ex.Message}");
            return Warning("QHY_PHOTOMETRY_RESUME_EXCEPTION_DEGRADED", $"{qhyUnavailableReason} ATR spectroscopy continues.");
        }
    }

    private async Task<StageResult> RestorePhotometryAfterResumeCoreAsync(
        ObservationContext context,
        CancellationToken cancellationToken)
    {
        if (photometryJobId is { } existing)
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (phd2.Snapshot.AppState is Phd2AppState.Looping or Phd2AppState.Selected)
        {
            ...
            var stopped = await phd2.StopCaptureAndConfirmAsync(cancellationToken).ConfigureAwait(false);
            ValidateConfirmedPhdStop(stopped, "Looping preview takeover");
        }
            ...
            ex.RejectedBeforeMutationState is Phd2AppState.Looping or Phd2AppState.Selected)
        {
            ...
            // state check before changing exposure or starting capture, so this
            // one recovery attempt follows a proven no-mutation rejection.
            Report("PHD2 在 G3 取帧前进入循环预览；确认停止后重发尚未执行的单次采集");
            var stopped = await phd2.StopCaptureAndConfirmAsync(cancellationToken).ConfigureAwait(false);
            ValidateConfirmedPhdStop(stopped, "late Looping preview takeover");
            return await phd2.CaptureFullFrameAsync(request, cancellationToken).ConfigureAwait(false);
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (phd2.Snapshot.AppState is Phd2AppState.Looping or Phd2AppState.Selected)
        {
            ...
            var stopped = await phd2.StopCaptureAndConfirmAsync(cancellationToken).ConfigureAwait(false);
            ValidateConfirmedPhdStop(stopped, "native single-frame Looping preview takeover");
        }
            ...
            ex.RejectedBeforeMutationState is Phd2AppState.Looping or Phd2AppState.Selected)
        {
            ...
            // made no exposure or capture mutation. Stop once, then dispatch
            // the still-unissued request once.
            Report("PHD2 在原生 G3 单帧下发前进入循环预览；确认停止后重发尚未执行的采集");
            var stopped = await phd2.StopCaptureAndConfirmAsync(cancellationToken).ConfigureAwait(false);
            ValidateConfirmedPhdStop(stopped, "late native single-frame Looping preview takeover");
            return await phd2.CaptureSingleFrameWithParametersAsync(request, cancellationToken).ConfigureAwait(false);
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            lease = await client.AcquireLeaseAsync(
                ...
                cancellationToken).ConfigureAwait(false);
            // A N.I.N.A. restart can leave the sole-owner UVEX service in a
            // recoverable Disconnected/Faulted state even though Night Setup
            // had connected it earlier. Slit discovery is an authorized part of
            // the active real run, so restore and verify COM5 before the first
            // idempotent OFF command instead of requiring a manual UI click.
            var connected = await lease.ConnectAndVerifyAsync(cancellationToken).ConfigureAwait(false);
            await WriteAuditBestEffortAsync("g3-slit-illumination-auto-connected", new
            {
                sequenceId,
                connected.PortName,
                connectionState = connected.ConnectionState.ToString(),
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            try
            {
                status = await lease.SetSlitIlluminationAsync(enabled, cancellationToken).ConfigureAwait(false);
            }
            catch (Exception firstFailure) when (!enabled)
            {
                // OFF is idempotent and is the safe recovery direction. If COM5
                // disappeared between connection verification and readback,
                // reconnect once and reassert OFF. Never apply this retry to ON.
                var reconnected = await lease.ConnectAndVerifyAsync(cancellationToken).ConfigureAwait(false);
                await WriteAuditBestEffortAsync("g3-slit-illumination-off-reconnected", new
                {
                    sequenceId = activeSlitIlluminationSequenceId,
                    phase,
                    firstFailure = firstFailure.Message,
                    connectionState = reconnected.ConnectionState.ToString(),
                    reconnected.PositionKnown,
                }).ConfigureAwait(false);
                status = await lease.SetSlitIlluminationAsync(enabled: false, cancellationToken).ConfigureAwait(false);
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            catch (Exception firstFailure)
            {
                try
                {
                    var reconnected = await activeSlitIlluminationLease
                        .ConnectAndVerifyAsync(cancellationToken)
                        .ConfigureAwait(false);
                    await WriteAuditBestEffortAsync("g3-slit-illumination-safety-off-reconnected", new
                    {
                        sequenceId,
                        reason,
                        firstFailure = firstFailure.Message,
                ...
                catch (Exception retryFailure)
                {
                    ...
                    await WriteAuditBestEffortAsync("g3-slit-illumination-off-unverified", new
                    {
                        sequenceId,
                        reason,
                        initialException = firstFailure.ToString(),
                        retryException = retryFailure.ToString(),
            ...
            // Output safety and control-lease cleanup are different facts.
            // Once OFF is command-completed and readback-verified, a failed
            // lease DELETE must not be re-labelled as an unknown LED output.
            Volatile.Write(ref slitIlluminationSafetyIssue, null);
            if (releaseIssue is not null)
            {
                Report($"warning：{releaseIssue}；LED OFF 已核验，后续操作会重新获取并核对租约");
                await WriteAuditBestEffortAsync("g3-slit-illumination-lease-release-warning", new
                {
                    sequenceId,
                    reason,
                    releaseIssue,
                    slitIlluminationOffVerified = true,
                    automaticResumeBlocked = false,
                }).ConfigureAwait(false);
            }
            return new SlitIlluminationOffAttempt(status, null);
        }
        finally
        {
            slitIlluminationGate.Release();
        ...
        bool allowMechanicalActions)
    {
        ...
        try
        {
            return await CleanupAfterFailureCoreAsync(
                reason,
                cancellationToken,
                allowMechanicalActions).ConfigureAwait(false);
        }
        finally
        {
            cleanupGate.Release();
        ...
        bool allowMechanicalActions)
    {
        ...
        if (allowMechanicalActions && configuration.Environment.CloseDomeOrRoofOnFailure)
        {
            cancellationToken.ThrowIfCancellationRequested();
            using var roofRecovery = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);
            roofRecovery.CancelAfter(TimeSpan.FromSeconds(configuration.Environment.DomeOrRoofTransitionTimeoutSeconds + 30));
            try
            {
                var roofIssue = await ParkMountAndCloseDomeOrRoofAsync(reason, roofRecovery.Token).ConfigureAwait(false);
                if (roofIssue is not null) failures.Add(roofIssue);
            }
            catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
            {
                throw;
            ...
            {
                failures.Add("Mount park / roll-off-roof close exceeded its bounded recovery timeout.");
            }
        }
        ...
        await WriteAuditBestEffortAsync("real-run-cleanup", new
        {
            ...
            roofCloseAttempted = allowMechanicalActions && configuration.Environment.CloseDomeOrRoofOnFailure,
        }).ConfigureAwait(false);
        PublishFrameCounters();
        return failures;
    }
    ...
    {
        // Coordinates represent a physical direction, not just two raw RA/Dec numbers.
        // N.I.N.A. accepts J2000 catalogue/WCS coordinates and converts them to the
        // mount's reported epoch (normally JNOW). Comparing those raw values directly
        // turns ordinary precession into a false pointing residual (about 1187 arcsec
        // for the 2026-08-29 QHY recovery incident). Always compare in one epoch.
        var bInAEpoch = b.Epoch == a.Epoch ? b : b.Transform(a.Epoch);
        var ra1 = a.RADegrees * Math.PI / 180d;
        var ra2 = bInAEpoch.RADegrees * Math.PI / 180d;
        var dec1 = a.Dec * Math.PI / 180d;
        var dec2 = bInAEpoch.Dec * Math.PI / 180d;
    ...
    private void Report(string message, double value = -1)
    {
        var culture = ObservationStaticTextLocalization.EffectiveCulture;
        var presentation = ObservationUiPresentation.PresentUiNotice(message, culture);
        progress.Report(new ApplicationStatus
        {
            Source = ObservationUiPresentation.Text("OpenAstroSpec Auto（真实）", "OpenAstroSpec Auto (real)", culture),
            Status = presentation.Message,
            Progress = value,
        });
    }
    ...
    private static StageResult Warning(
        string code,
        string message,
        IReadOnlyDictionary<string, double>? metrics = null,
        IReadOnlyDictionary<string, string>? metadata = null) =>
        new(GateResult.Warn(code, message, metrics), Metadata: metadata);
...
internal sealed record G3SlitGeometryRunCache(
    ...
    int Binning,
    int MeasuredAtFocusPositionSteps,
...
internal sealed record G3WcsMotionPrediction(
    PixelPoint PredictedTargetPoint,
    double MaximumUncertaintyPixels,
    string SourceFramePath,
    string SourceSolveEvidencePath,
    string SourceMountBindingSha256,
    double CommandedRaDegrees,
    double CommandedDecDegrees,
