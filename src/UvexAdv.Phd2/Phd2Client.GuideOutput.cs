        ...
        Phd2IdentityRequirement expected, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (Snapshot.GuideOutput?.Failed != true || Snapshot.Phd2Paused ||
                await GetAppStateAsync(cancellationToken).ConfigureAwait(false) is not (Phd2AppState.Stopped or Phd2AppState.Selected))
            ...
            ThrowIfAutomationPaused();
            if (await GetAppStateAsync(cancellationToken).ConfigureAwait(false) is not (Phd2AppState.Stopped or Phd2AppState.Selected))
            ...
            var disconnected = await GetCurrentEquipmentAsync(cancellationToken).ConfigureAwait(false);
